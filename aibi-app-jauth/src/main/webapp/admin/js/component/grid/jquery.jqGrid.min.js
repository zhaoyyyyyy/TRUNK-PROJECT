/*
 * jqGrid 3.8.2 - jQuery Grid Copyright (c) 2008, Tony Tomov, tony@trirand.com
 * Dual licensed under the MIT and GPL licenses
 * http://www.opensource.org/licenses/mit-license.php
 * http://www.gnu.org/licenses/gpl-2.0.html Date:2010-12-14 Modules:
 * grid.base.js; jquery.fmatter.js; grid.custom.js; grid.common.js;
 * grid.formedit.js; jquery.searchFilter.js; grid.inlinedit.js;
 * grid.celledit.js; jqModal.js; jqDnR.js; grid.subgrid.js; grid.grouping.js;
 * grid.treegrid.js; grid.import.js; JsonXml.js; grid.setcolumns.js;
 * grid.postext.js; grid.tbltogrid.js; grid.jqueryui.js;
 * 
 * 2011-04-27 zhouyingzhao 修改了rowList的设置方式。
 * 2011-05-06 zhouyingzhao 新加了一个print按钮，使用navAddButton接口不太好统一，需要留的接口太多，所以使用了。
 */
(function(b) {
	b.jgrid = b.jgrid || {};
	b.extend(b.jgrid, {
		...
		parseDate : function(f, j) {
			...
			if (j && j !== null && j !== undefined) {
				...
				j = j.split(/[\\\/:_;.\t\T\s-]/);
				f = f.split(/[\\\/:_;.\t\T\s-]/);
				var n = b.jgrid.formatter.date.monthNames, a = b.jgrid.formatter.date.AmPm, r = function(
						t, B) {
					if (t === 0) {
						if (B == 12)
							B = 0
		...
		from : function(f) {
			return new (function(j, i) {
				if (typeof j == "string")
					j = b.data(j);
				var c = this, e = j, k = true, n = false, a = i, r = /[\$,%]/g, t = null, B = null, F = false, S = "", J = [], N = true;
				if (typeof j == "object" && j.push) {
					if (j.length > 0)
						N = typeof j[0] != "object" ? false : true
				} else
				...
				this._doSort = function(l, m) {
					var q = J[m].by, A = J[m].dir, u = J[m].type, I = J[m].datefmt;
					if (m == J.length - 1)
						return c._getOrder(l, q, A, u, I);
					m++;
				...
				this._getOrder = function(l, m, q, A, u) {
					var I = [], U = [], $ = q == "a" ? 1 : -1, P, aa;
					if (A === undefined)
						A = "text";
					aa = A == "float" || A == "number" || A == "currency"
							|| A == "numeric" ? function(L) {
						L = parseFloat(String(L).replace(r, ""));
						return isNaN(L) ? 0 : L
					} : A == "int" || A == "integer" ? function(L) {
						return L ? parseFloat(String(L).replace(r, "")) : 0
					} : A == "date" || A == "datetime" ? function(L) {
				...
				this._compareValues = function(l, m, q, A, u) {
					...
					switch (u.stype === undefined ? "text" : u.stype) {
						...
						case "datetime" :
							q = String(b.jgrid
									.parseDate(u.newfmt || "Y-m-d", q)
									.getTime());
							I = 'jQuery.jgrid.parseDate("' + u.srcfmt + '",'
									+ I + ").getTime()";
							break;
						default :
							I = c._getStr(I);
				...
				this.startsWith = function(l, m) {
					...
					if (N)
						c._append(c._getStr("this." + l) + ".substr(0," + q
								+ ") == " + c._getStr('"' + c._toStr(m) + '"'));
					else {
						q = n
								? b.trim(m.toString()).length
								: m.toString().length;
						c._append(c._getStr("this") + ".substr(0," + q
				...
				this.endsWith = function(l, m) {
					var q = m === undefined || m === null ? l : m;
					q = n ? b.trim(q.toString()).length : q.toString().length;
					N ? c._append(c._getStr("this." + l) + ".substr("
							+ c._getStr("this." + l) + ".length-" + q + "," + q
							+ ') == "' + c._toStr(m) + '"') : c._append(c
							._getStr("this")
							+ ".substr("
							+ c._getStr("this")
							+ '.length-"'
				...
				this.contains = function(l, m) {
					N ? c._append(c._getStr("this." + l) + '.indexOf("'
							+ c._toStr(m) + '",0) > -1') : c._append(c
							._getStr("this")
							+ '.indexOf("' + c._toStr(l) + '",0) > -1');
				...
				this.orderBy = function(l, m, q, A) {
					m = m === undefined || m === null ? "a" : b.trim(m
							.toString().toLowerCase());
					if (q === null || q === undefined)
						q = "text";
					if (A === null || A === undefined)
						A = "Y-m-d";
					if (m == "desc" || m == "descending")
						m = "d";
					if (m == "asc" || m == "ascending")
						m = "a";
					J.push({
								by : l,
								dir : m,
	...
	b.fn.jqGrid = function(f) {
			...
			if (!this.grid) {
				var c = b.extend(true, {
					...
					dragEnd : function() {
						...
						if (this.resizing) {
							...
							if (c.forceFit === true) {
								...
								this.headers[d + c.nv].el.style.width = g
										+ "px";
								this.cols[d + c.nv].style.width = g + "px";
								if (this.footers.length > 0)
									this.footers[d + c.nv].style.width = g
											+ "px";
					...
					populateVisible : function() {
						...
						if (d) {
							var g = b("table:first", e.bDiv), h = b(
									"> tbody > tr:gt(0):visible:first", g)
									.outerHeight()
									|| e.prevRowHeight;
							if (h) {
								e.prevRowHeight = h;
								var o = c.rowNum, p = e.scrollTop = e.bDiv.scrollTop, x = Math
					...
					selectionPreserver : function(d) {
						var g = d.p, h = g.selrow, o = g.selarrrow ? b
								.makeArray(g.selarrrow) : null, p = d.grid.bDiv.scrollLeft, x = g.gridComplete;
						g.gridComplete = function() {
							g.selrow = null;
							g.selarrrow = [];
							if (g.multiselect && o && o.length > 0)
								for (var w = 0; w < o.length; w++)
									o[w] != h
											&& b(d).jqGrid("setSelection",
													o[w], false);
				...
				/** add by zhouyingzhao 2011-04-27, default中设置了rowList之后，自己设置的rowList少的话，不起作用。 */
				if (f.rowList && jQuery.isArray(f.rowList)) {
					c.rowList = f.rowList;
				}
				...
				if (this.tagName != "TABLE")
					...
					if (this.p.colNames.length !== this.p.colModel.length)
						...
						var t = b("<div class='ui-jqgrid-view'></div>"), B, F = b.browser.msie
								? true
								: false, S = b.browser.safari ? true : false;
						a = this;
						...
						var N = function(d, g) {
							...
							var o = a.p.colModel[d], p = o.align, x = 'style="', w = o.classes, C = o.name;
							if (p)
								x += "text-align:" + p + ";";
							if (o.hidden === true)
								x += "display:none;";
							...
							if (typeof x.formatter !== "undefined") {
								...
								g = b.isFunction(x.formatter) ? x.formatter
										.call(a, g, d, o, p) : b.fmatter
										? b.fn.fmatter(x.formatter, g, d, o, p)
										: m(g)
							} else
								g = m(g);
							return g
							...
							d = q(d, g, h, p, "add");
							return '<td role="gridcell" ' + l(h, o, d) + ">"
									+ d + "</td>"
						}, u = function(d, g, h) {
							...
							if (!g || !g.length)
								g = b.map(a.p.colModel, function(h, o) {
											return o
										});
							if (d)
							...
							if (a.p.deepempty)
								b(		"#" + a.p.id + " tbody:first tr:gt(0)")
										.remove();
							else {
								var h = b("#" + a.p.id
										+ " tbody:first tr:first")[0];
								b("#" + a.p.id + " tbody:first").empty()
										.append(h)
							}
							if (g && a.p.scroll) {
								b(">div:first", d).css({
							...
							var d = a.p.data.length, g, h, o;
							g = a.p.rownumbers === true ? 1 : 0;
							h = a.p.multiselect === true ? 1 : 0;
							o = a.p.subGrid === true ? 1 : 0;
							g = a.p.keyIndex === false || a.p.loadonce === true
									? a.p.localReader.id
									: a.p.colModel[a.p.keyIndex + h + o + g].name;
							for (h = 0; h < d; h++) {
								o = b.jgrid.getAccessor(a.p.data[h], g);
								a.p._index[o] = h
							}
						}, ea = function(d, g, h, o, p) {
							var x = new Date, w = a.p.datatype != "local"
									&& a.p.loadonce
									|| a.p.datatype == "xmlstring", C, G = a.p.datatype == "local"
									? "local"
									: "xml";
							if (w) {
								a.p.data = [];
							...
							if (b.isXMLDoc(d)) {
								if (a.p.treeANode === -1 && !a.p.scroll) {
									...
									h = h > 1 ? h : 1;
								var s, v = 0, y, D, K = 0, H = 0, E = 0, Q, z = [], W, T = {}, O, M, X = [], na = a.p.altRows === true
										? " " + a.p.altclass
										: "";
								...
								if (z.length > 0 && !isNaN(Q)) {
									if (a.p.remapColumns
										...
										Q = b.inArray(Q, a.p.remapColumns);
									Q = z[Q]
								}
								G = (Q + "").indexOf("[") === -1 ? z.length
										...
										: function(ca, Y) {
											return b(a.p.xmlReader.cell, ca)
													.eq(Q).text()
													|| Y
										} : function(ca, Y) {
									return ca.getAttribute(Q.replace(/[\[\]]/g,
											""))
											|| Y
								};
								a.p.userData = {};
								b(a.p.xmlReader.page, d).each(function() {
									a.p.page = this.textContent || this.text
											|| 0
								});
								b(a.p.xmlReader.total, d).each(function() {
									a.p.lastpage = this.textContent
											|| this.text;
									if (a.p.lastpage === undefined)
										a.p.lastpage = 1
								});
								b(a.p.xmlReader.records, d).each(function() {
									a.p.records = this.textContent || this.text
											|| 0
								});
								...
								(d = b(	a.p.xmlReader.root + " "
												+ a.p.xmlReader.row, d))
										|| (d = []);
								var fa = d.length, Z = 0;
								if (d && fa) {
									var ga = parseInt(a.p.rowNum, 10), qa = a.p.scroll
											? (parseInt(a.p.page, 10) - 1) * ga
													+ 1
											: 1;
									if (p)
										ga *= p + 1;
									...
									for (; Z < fa;) {
										...
										if (a.p.grouping) {
											...
											for (var ya = 0; ya < s; ya++)
												D
														.push(T[a.p.groupingView.groupField[ya]]);
											ha = b(a).jqGrid("groupingPrepare",
													X, D, ha, T);
											X = []
										}
										if (w) {
							...
							if (d) {
								...
								var s = 0, v, y, D, K = [], H, E = 0, Q = 0, z = 0, W, T, O = {}, M;
								D = [];
								var X = a.p.altRows === true ? " "
										+ a.p.altclass : "";
								a.p.page = b.jgrid.getAccessor(d, g.page) || 0;
								W = b.jgrid.getAccessor(d, g.total);
								a.p.lastpage = W === undefined ? 1 : W;
								a.p.records = b.jgrid.getAccessor(d, g.records)
										|| 0;
								a.p.userData = b.jgrid.getAccessor(d,
										g.userdata)
										|| {};
								g.repeatitems || (H = K = U(C));
								C = a.p.keyIndex === false
										? g.id
										: a.p.keyIndex;
								...
								(T = b.jgrid.getAccessor(d, g.root))
										|| (T = []);
								W = T.length;
								d = 0;
								var na = parseInt(a.p.rowNum, 10), fa = a.p.scroll
										? (parseInt(a.p.page, 10) - 1) * na + 1
										: 1;
								if (p)
									na *= p + 1;
								var Z = b.isFunction(a.p.afterInsertRow), ga = {}, qa = "";
								if (a.p.grouping
										&& a.p.groupingView.groupCollapse === true)
								...
								for (; d < W;) {
									...
									if (a.p.subGrid) {
										D.push(b(a).jqGrid("addSubGridCell",
												E + z, d + h));
										Q = 1
									}
									if (g.repeatitems) {
										if (g.cell)
											p = b.jgrid.getAccessor(p, g.cell);
										H || (H = $(E + Q + z))
									}
									for (y = 0; y < H.length; y++) {
										v = b.jgrid.getAccessor(p, H[y]);
										D
												.push(A(M, v, y + E + Q + z, d
																+ h, p));
									...
									if (a.p.grouping) {
										...
										for (var ha = 0; ha < v; ha++)
											y
													.push(O[a.p.groupingView.groupField[ha]]);
										ga = b(a).jqGrid("groupingPrepare", D,
												y, ga, O);
										D = []
									}
									if (G) {
							...
							if (b.isArray(a.p.data)) {
								...
								if (a.p.treeGrid)
									b(a).jqGrid("SortTree", d, a.p.sortorder,
											h[d].stype, h[d].srcfmt);
								else {
									var s = {
										eq : function(z) {
											return z.equals
									...
									if (a.p.grouping) {
										v.orderBy(p, G.groupOrder[0],
												o[0].stype, o[0].srcfmt);
										G.groupDataSorted = true
									}
									if (d && a.p.sortorder && g)
										a.p.sortorder.toUpperCase() == "DESC"
												? v
							...
							if (!a.grid.hDiv.loading) {
								var g = a.p.scroll && d === false, h = {}, o, p = a.p.prmNames;
								if (a.p.page <= 0)
									a.p.page = 1;
								if (p.search !== null)
									h[p.search] = a.p.search;
								if (p.nd !== null)
									h[p.nd] = (new Date).getTime();
								if (p.rows !== null)
									h[p.rows] = a.p.rowNum;
								if (p.page !== null)
									h[p.page] = a.p.page;
								if (p.sort !== null)
									h[p.sort] = a.p.sortname;
								if (p.order !== null)
									h[p.order] = a.p.sortorder;
								if (a.p.rowTotal !== null
										&& p.totalrows !== null)
									h[p.totalrows] = a.p.rowTotal;
								...
								if (a.p.grouping) {
									...
									if (a.p.groupingView.groupDataSorted === true)
										h[p.sort] = a.p.groupingView.groupField[0]
												+ " "
								...
								if (b.isFunction(a.p.datatype))
									a.p.datatype.call(a, a.p.postData, "load_"
													+ a.p.id);
								else {
											...
											b.ajax(b.extend(
													{
														...
														dataType : o,
														data : b
																.isFunction(a.p.serializeGridData)
																? a.p.serializeGridData
																		.call(
																				a,
																				a.p.postData)
																: a.p.postData,
						...
						B = function(d, g) {
							var h = "", o = "<table cellspacing='0' cellpadding='0' border='0' style='table-layout:auto;' class='ui-pg-table'><tbody><tr>", p = "", x, w, C, G, s = function(
									v) {
								var y;
								...
								if (a.p.multiselect) {
									...
									b("#cb_" + b.jgrid.jqID(a.p.id),
											a.grid.hDiv).attr("checked", false)
								}
								a.p.savedRow = [];
								if (y == "stop")
									return false;
							...
							a.p._nvtd = [];
							a.p._nvtd[0] = o
									? Math.floor((a.p.width - o) / 2)
									: Math.floor(a.p.width / 3);
							a.p._nvtd[1] = 0;
							o = null;
							b(".ui-pg-selbox", "#" + x).bind("change",
									function() {
										a.p.page = Math.round(a.p.rowNum
												* (a.p.page - 1) / this.value
												- 0.5)
												+ 1;
										a.p.rowNum = this.value;
										if (g)
											b(".ui-pg-selbox", a.p.pager)
													.val(this.value);
										else
											a.p.toppager
													&& b(".ui-pg-selbox",
							...
							if (a.p.pgbuttons === true) {
								...
								b(
										"#first" + g + ", #prev" + g
												+ ", #next" + g + ", #last" + g,
										"#" + d).click(function() {
									var v = N(a.p.page, 1), y = N(a.p.lastpage,
											1), D = false, K = true, H = true, E = true, Q = true;
									if (y === 0 || y === 1)
										Q = E = H = K = false;
									else if (y > 1 && v >= 1)
										if (v === 1)
						...
						var Ca = function(d, g, h, o) {
							if (a.p.colModel[g].sortable)
								if (!(a.p.savedRow.length > 0)) {
									...
									b(
											"tr th:eq(" + a.p.lastsort
													+ ") span.ui-grid-ico-sort",
											h).addClass("ui-state-disabled");
									b("tr th:eq(" + a.p.lastsort + ")", h)
											.attr("aria-selected", "false");
									b(
											"tr th:eq(" + g + ") span.ui-icon-"
													+ a.p.sortorder, h)
								...
								h[1] += b(a.grid.topDiv).height()
										+ parseInt(
												b(a.grid.topDiv)
														.css("border-bottom-width"),
												10);
							h[2] += b(a.grid.bDiv).height()
									+ b(a.grid.hDiv).height();
							return h
						};
						this.p.id = this.id;
						if (b.inArray(a.p.multikey, ["shiftKey", "altKey",
										"ctrlKey"]) == -1)
							a.p.multikey = false;
						a.p.keyIndex = false;
						...
						ma = "<span class='s-ico' style='display:none'><span sort='asc' class='ui-grid-ico-sort ui-icon-asc"
								+ ma
								+ " ui-state-disabled ui-icon ui-icon-triangle-1-n ui-sort-"
								+ n + "'></span>";
						ma += "<span sort='desc' class='ui-grid-ico-sort ui-icon-desc"
								+ la
						...
						for (k = 0; k < this.p.colNames.length; k++) {
							...
							r += "<div id='jqgh_" + a.p.colModel[k].name + "' "
									+ Da + ">" + a.p.colNames[k];
							a.p.colModel[k].width = a.p.colModel[k].width
									? parseInt(a.p.colModel[k].width, 10)
									: 150;
		...
		(function		() {
							var d = 0, g = a.p.cellLayout, h = 0, o, p = a.p.scrollOffset, x, w = false, C, G = 0, s = 0, v = 0, y;
							if (S)
								g = 0;
							b.each(a.p.colModel, function() {
							...
							if (a.p.shrinkToFit === true && h > 0) {
								...
								y = 0;
								if (w) {
									if (e.width - s - (d + g * h) !== p)
										y = e.width - s - (d + g * h) - p
								} else if (!w
										&& Math.abs(e.width - s - (d + g * h)) !== 1)
									y = e.width - s - (d + g * h);
								a.p.colModel[o].width += y;
								a.p.tblwidth = d + y + G + v * g;
								if (a.p.tblwidth > a.p.width) {
									a.p.colModel[o].width -= a.p.tblwidth
						...
						b("th", t).each(function(d) {
							...
							var g = "th>div.ui-jqgrid-sortable", h, o;
							a.p.viewsortcols[2]
						...
						wa = null;
						t = b("<table class='ui-jqgrid-htable' style='width:"
								+ a.p.tblwidth
								+ "px' role='grid' aria-labelledby='gbox_"
								+ this.id
								+ "' cellspacing='0' cellpadding='0' border='0'></table>")
								.append(r);
						var ba = a.p.caption && a.p.hiddengrid === true
								? true
								: false;
						...
						a.p.cellEdit === false && a.p.hoverrows === true
								&& b(a).bind("mouseover", function(d) {
							...
							if (h === true) {
								if (a.p.cellEdit === true)
									if (a.p.multiselect && g)
										b(a).jqGrid("setSelection", R[0].id,
												true);
									else {
										ja = R[0].rowIndex;
										ka = b.jgrid.getCellIndex(V);
										try {
											b(a).jqGrid("editCell", ja, ka,
													true)
										} catch (o) {
										}
									}
								...
								if (b.isFunction(a.p.onCellSelect)) {
									ja = R[0].id;
									ka = b.jgrid.getCellIndex(V);
									a.p.onCellSelect.call(a, ja, ka, b(V)
													.html(), d)
								}
								d.stopPropagation()
							} else
						...
						b.isFunction(this.p.ondblClickRow)
								&& b(this).dblclick(function(d) {
									...
									ja = R[0].rowIndex;
									ka = b.jgrid.getCellIndex(V);
									a.p.ondblClickRow.call(a, b(R).attr("id"),
											ja, ka, d);
									return false
								});
						b.isFunction(this.p.onRightClickRow)
								&& b(this).bind("contextmenu", function(d) {
									V = d.target;
									R = b(V, a.rows).closest("tr.jqgrow");
									if (b(R).length === 0)
										return false;
									a.p.multiselect
											|| b(a).jqGrid("setSelection",
													R[0].id, true);
									ja = R[0].rowIndex;
									ka = b.jgrid.getCellIndex(V);
									a.p.onRightClickRow.call(a,
											b(R).attr("id"), ja, ka, d);
									return false
								});
						e.bDiv = document.createElement("div");
						// modify by周营昭， 在ie下，有一个边框去不掉。
						b(e.bDiv).append(b('<div style="position:relative;top:-1px;'
								+ (F && b.browser.version < 8
										? "height:0.01%;"
										: "") + '"></div>')
						...
						if (a.p.toppager) {
							...
							e.topDiv = b("<div id='" + a.p.toppager
									+ "'></div>")[0];
						...
						if (a.p.footerrow) {
							...
							k = b("<div class='ui-jqgrid-hbox"
									+ (n == "rtl" ? "-rtl" : "") + "'></div>");
							b(e.sDiv).append(k).insertAfter(e.hDiv)
									.width(e.width);
							b(k).append(va);
						...
						if (a.p.caption) {
							...
							if (a.p.hidegrid === true) {
								b(".ui-jqgrid-titlebar-close", e.cDiv).click(
										function(d) {
											var g = b
													.isFunction(a.p.onHeaderClick), h = ".ui-jqgrid-bdiv, .ui-jqgrid-hdiv, .ui-jqgrid-pager, .ui-jqgrid-sdiv", o, p = this;
											if (a.p.toolbar[0] === true) {
												if (a.p.toolbar[1] == "both")
													h += ", #"
															+ b(e.ubDiv)
								...
								if (ba) {
									a.p.datatype = "local";
									b(".ui-jqgrid-titlebar-close", e.cDiv)
											.trigger("click")
								}
							}
						} else
						...
						b(".ui-jqgrid-labels", e.hDiv).bind("selectstart",
								function() {
									return false
								});
						...
						a.updatepager = function(d, g) {
							...
							if (s) {
								...
								if (a.p.pginput === true) {
									...
									b("#sp_1", s).html(b.fmatter
											? b.fmatter.util.NumberFormat(
													a.p.lastpage, G)
											: a.p.lastpage)
								}
								if (a.p.viewrecords)
									if (a.p.reccount === 0)
										...
										C = a.p.records;
										if (b.fmatter) {
											x = b.fmatter.util.NumberFormat(x,
													G);
											w = b.fmatter.util.NumberFormat(w,
													G);
											C = b.fmatter.util.NumberFormat(C,
													G)
										}
										b(".ui-paging-info", s)
												.html(b.jgrid
														.format(a.p.recordtext,
																x, w, C))
									}
								if (a.p.pgbuttons === true) {
									if (h <= 0)
							...
							d === true
									&& a.p.rownumbers === true
									&& b("td.jqgrid-rownum", a.rows).each(
											function(v) {
												b(this).html(p + 1 + v)
											});
							g && a.p.jqgdnd
	...
	b.jgrid.extend({
		...
		setGridParam : function(f) {
			return this.each(function() {
						this.grid && typeof f === "object"
								&& b.extend(true, this.p, f)
					})
		},
		getDataIDs : function() {
			var f = [], j = 0, i, c = 0;
		...
		setSelection : function(f, j) {
			return this.each(function() {
				function i(n) {
					var a = b(c.grid.bDiv)[0].clientHeight, r = b(c.grid.bDiv)[0].scrollTop, t = c.rows[n].offsetTop;
					n = c.rows[n].clientHeight;
					if (t + n >= a + r)
						b(c.grid.bDiv)[0].scrollTop = t - (a + r) + n + r;
					else if (t < a + r)
						if (t < r)
							b(c.grid.bDiv)[0].scrollTop = t
				}
				var c = this, e, k;
				if (f !== undefined) {
						...
						if (c.p.multiselect) {
							...
							if (k === -1) {
								e.className !== "ui-subgrid"
										&& b(e).addClass("ui-state-highlight")
												.attr("aria-selected", "true");
								e = true;
								b("#jqg_" + c.p.id + "_"
										+ b.jgrid.jqID(c.p.selrow)).attr(
										"checked", e);
								c.p.selarrrow.push(c.p.selrow);
								c.p.onSelectRow
										&& j
										&& c.p.onSelectRow.call(c, c.p.selrow,
												e)
							} else {
								e.className !== "ui-subgrid"
										&& b(e)
						...
						if (f.p.multiselect) {
							b(f.p.selarrrow).each(function(i, c) {
								...
								b(j).removeClass("ui-state-highlight").attr(
										"aria-selected", "false");
								b("#jqg_" + f.p.id + "_" + b.jgrid.jqID(c))
										.attr("checked", false)
							});
							b("#cb_" + b.jgrid.jqID(f.p.id)).attr("checked",
									false);
							f.p.selarrrow = []
						} else if (f.p.selrow) {
							b("#" + f.p.id + " tbody:first tr#"
									+ b.jgrid.jqID(f.p.selrow))
						...
						for (; k < e;) {
							...
							if (b(r).hasClass("jqgrow")) {
								b("td", r).each(function(t) {
									...
									if (a !== "cb" && a !== "subgrid"
											&& a !== "rn")
										if (n.p.treeGrid === true
												&& a == n.p.ExpandColumn)
											j[a] = b.jgrid.htmlDecode(b(
			...
			this.each(function() {
						...
						if (e.p.altRows === true && j) {
							...
							b(e.rows).each(function(a) {
								a % 2 == 1 ? b(this).addClass(n) : b(this)
										.removeClass(n)
							})
						}
					});
		...
		addRowData : function(f, j, i, c) {
			i || (i = "last");
			var e = false, k, n, a, r, t, B, F, S, J = "", N, l, m, q, A;
			if (j) {
				if (b.isArray(j)) {
					N = true;
					i = "last";
				...
				this.each(function() {
					...
					if (!N)
						if (typeof f != "undefined")
							...
							if (u.p.keyIndex !== false) {
								...
								if (typeof j[0][l] != "undefined")
									f = j[0][l]
							}
						}
					m = u.p.altclass;
					for (var U = 0, $ = "", P = {}, aa = b
							.isFunction(u.p.afterInsertRow) ? true : false; U < I;) {
						q = j[U];
						n = "";
						if (N) {
							try {
							...
							$ = u.p.altRows === true
									? (u.rows.length - 1) % 2 === 0 ? m : ""
									: ""
						}
						if (t) {
							J = u.formatCol(0, 1, "");
							n += '<td role="gridcell" aria-describedby="'
									+ u.p.id
									+ '_rn" class="ui-state-default jqgrid-rownum" '
									+ J + ">0</td>"
						}
						if (a) {
							S = '<input role="checkbox" type="checkbox" id="jqg_'
									+ u.p.id + "_" + f + '" class="cbox"/>';
							J = u.formatCol(t, 1, "");
							n += '<td role="gridcell" aria-describedby="'
									+ u.p.id + '_cb" ' + J + ">" + S + "</td>"
						}
						if (r)
							n += b(u).jqGrid("addSubGridCell", a + t, 1);
						for (F = a + r + t; F < u.p.colModel.length; F++) {
							A = u.p.colModel[F];
							k = A.name;
							P[k] = A.formatter
									&& typeof A.formatter === "string"
									&& A.formatter == "date" ? b.unformat.date(
									q[k], A) : q[k];
							S = u.formatter(f, b.jgrid.getAccessor(q, k), F, q,
									"edit");
							J = u.formatCol(F, 1, S);
							n += '<td role="gridcell" aria-describedby="'
									+ u.p.id + "_" + k + '" ' + J + ">" + S
						...
						if (u.rows.length === 0)
							b("table:first", u.grid.bDiv).append(n);
						else
							switch (i) {
								case "last" :
					...
					if (u.p.altRows === true && !N)
						if (i == "last")
							(u.rows.length - 1) % 2 == 1
									&& b(u.rows[u.rows.length - 1]).addClass(m);
						else
							b(u.rows).each(function(L) {
								L % 2 == 1 ? b(this).addClass(m) : b(this)
										.removeClass(m)
							});
						...
						b(this.p.colModel).each(function(B) {
							...
							if (f == "set") {
								if (j[e] !== undefined) {
									...
									b("tr.footrow td:eq(" + B + ")",
											r.grid.sDiv).html(t).attr(a);
									k = true
								}
							} else if (f == "get")
								n[e] = b("tr.footrow td:eq(" + B + ")",
										r.grid.sDiv).html()
						})
					});
			return f == "get" ? n : k
				...
				if (i.grid) {
					...
					b(this.p.colModel).each(function(k) {
						if (b.inArray(this.name, f) !== -1 && this.hidden === e) {
							...
							b(i.rows).each(function(n) {
								b("td:eq(" + k + ")", i.rows[n]).css("display",
										j)
							});
							i.p.footerrow
									&& b("td:eq(" + k + ")", i.grid.sDiv).css(
											"display", j);
							if (j == "none")
					...
					if (c === true) {
						...
						i.p.shrinkToFit === true
								&& b(i).jqGrid("setGridWidth",
										i.grid.width - 0.001, true)
					}
				}
			})
		},
		...
		remapColumns : function(f, j, i) {
			function c(n) {
				var a;
				a = n.length ? b.makeArray(n) : b.extend({}, n);
				b.each(f, function(r) {
							n[r] = a[this]
						})
			}
			function e(n, a) {
				b(">tr" + (a || ""), n).each(function() {
							var r = this, t = b.makeArray(r.cells);
							b.each(f, function() {
										var B = t[this];
										B && r.appendChild(B)
									})
		...
		setGridWidth : function(f, j) {
			return this.each(function() {
				if (this.grid) {
					var i = this, c, e = 0, k = i.p.cellLayout, n, a = 0, r = false, t = i.p.scrollOffset, B, F = 0, S = 0, J = 0, N;
					if (typeof j != "boolean")
						j = i.p.shrinkToFit;
					if (!isNaN(f)) {
						f = parseInt(f, 10);
								...
								b.each(i.p.colModel, function(m) {
									if (this.hidden === false && !this.fixed) {
										...
										if (!(c < 0)) {
											...
											i.grid.headers[m].el.style.width = c
													+ "px";
											if (i.p.footerrow)
												i.grid.footers[m].style.width = c
														+ "px";
											if (l)
												i.grid.cols[m].style.width = c
														+ "px";
											n = m
										}
									}
			...
			return this.each(function() {
						...
						if (e.grid) {
							if (isNaN(f))
								...
								k = parseInt(f, 10);
							if (k >= 0) {
								var n = b("tr.ui-jqgrid-labels th:eq(" + k
												+ ")", e.grid.hDiv);
								if (j) {
									var a = b(".s-ico", n);
									b("[id^=jqgh_]", n).empty().html(j)
											.append(a);
									e.p.colNames[k] = j
								}
								if (i)
		...
		setCell : function(f, j, i, c, e, k) {
			return this.each(function() {
						...
						if (n.grid) {
							if (isNaN(j))
								b(n.p.colModel).each(function(F) {
											if (this.name == j) {
												a = F;
												return false
											}
							...
							for (; t < r;) {
								if (b(n.rows[t]).hasClass("jqgrow")) {
									try {
										...
										e = b.jgrid
												.htmlDecode(n.rows[t].cells[a].innerHTML)
									}
									if (i)
										k += parseFloat(e);
									else if (j)
			...
			return this.each(function() {
						...
						if (j.grid) {
							if (typeof f != "boolean")
								f = false;
							if (j.p.deepempty)
								b("#" + j.p.id + " tbody:first tr:gt(0)")
										.remove();
							else {
								var i = b("#" + j.p.id
										+ " tbody:first tr:first")[0];
								b("#" + j.p.id + " tbody:first").empty()
										.append(i)
							}
							j.p.footerrow
									&& f
									&& b(".ui-jqgrid-ftable td", j.grid.sDiv)
											.html("&#160;");
							j.p.selrow = null;
							j.p.selarrrow = [];
							j.p.savedRow = [];
...
(function(c) {
	...
	c.extend(c.fmatter, {
				...
				isObject : function(a) {
					return a && (typeof a === "object" || c.isFunction(a))
							|| false
				},
				isString : function(a) {
				...
				isEmpty : function(a) {
					if (!this.isString(a) && this.isValue(a))
						...
						return true;
					a = c.trim(a).replace(/\&nbsp\;/ig, "").replace(
							/\&#160\;/ig, "");
					return a === ""
				}
			});
	...
	c.fmatter.util = {
		NumberFormat : function(a, b) {
			...
			if (c.fmatter.isNumber(a)) {
				var d = a < 0, f = a + "", e = b.decimalSeparator
						? b.decimalSeparator
						: ".";
		...
		DateFormat : function(a, b, d, f) {
			var e = b
					.match(/^\/Date((([-+])?[0-9]+)(([-+])([0-9]{2})([0-9]{2}))?)\/$/), i = function(
					m, r) {
				m = String(m);
			...
			if (b.constructor === Number)
				...
				for (j = a.length; e < j; e++) {
					...
					if (b[e])
						g[a[e].toLowerCase()] = parseInt(b[e], 10)
				}
				if (g.f)
					g.m = g.f;
			...
			var l = h.getSeconds(), o = h.getMilliseconds(), n = h.getDay(), p = h
					.getFullYear(), q = (n + 6) % 7 + 1, s = (new Date(p,
					e - 1, b) - new Date(p, 0, 1))
					/ 864E5, t = {
				d : i(b),
				D : k.i18n.dayNames[n],
				j : b,
				...
				I : "?",
				O : (j > 0 ? "-" : "+")
						+ i(	Math.floor(Math.abs(j) / 60) * 100
										+ Math.abs(j) % 60, 4),
				P : "?",
				T : (String(h)
						.match(/\b(?:[PMCEA][SDP]T|(?:Pacific|Mountain|Central|Eastern|Atlantic) (?:Standard|Daylight|Prevailing) Time|(?:GMT|UTC)(?:[-+]\d{4})?)\b/g) || [""])
						.pop().replace(/[^-+\dA-Z]/g, ""),
				Z : "?",
				c : "?",
				r : "?",
	...
	c.fn.fmatter.defaultFormat = function(a, b) {
		return c.fmatter.isValue(a) && a !== "" ? a : b.defaultValue
				? b.defaultValue
				: "&#160;"
	};
	c.fn.fmatter.email = function(a, b) {
		return c.fmatter.isEmpty(a)
				? c.fn.fmatter.defaultFormat(a, b)
				: '<a href="mailto:' + a + '">' + a + "</a>"
	};
	...
	c.fn.fmatter.link = function(a, b) {
		...
		if (d.target)
			f = "target=" + d.target;
		return c.fmatter.isEmpty(a) ? c.fn.fmatter.defaultFormat(a, b) : "<a "
				+ f + ' href="' + a + '">' + a + "</a>"
	};
	c.fn.fmatter.showlink = function(a, b) {
		...
		d = d.baseLinkUrl + d.showAction + "?" + d.idName + "=" + b.rowId
				+ d.addParam;
		return c.fmatter.isString(a) || c.fmatter.isNumber(a) ? "<a " + f
				+ ' href="' + d + '">' + a + "</a>" : c.fn.fmatter
				.defaultFormat(a, b)
	};
	...
	c.fn.fmatter.date = function(a, b, d, f) {
		...
		c.fmatter.isUndefined(b.colModel.formatoptions)
				|| (d = c.extend({}, d, b.colModel.formatoptions));
		return !d.reformatAfterEdit && f == "edit" ? c.fn.fmatter
				.defaultFormat(a, b) : c.fmatter.isEmpty(a) ? c.fn.fmatter
				.defaultFormat(a, b) : c.fmatter.util.DateFormat(d.srcformat,
				a, d.newformat, d)
	};
	c.fn.fmatter.select = function(a, b) {
		...
		if (d) {
			var e = b.colModel.editoptions.multiple === true ? true : false, i = [], g;
			if (e) {
				i = a.split(",");
			...
			if (c.fmatter.isString(d))
				for (var h = d.split(";"), j = 0, k = 0; k < h.length; k++) {
					g = h[k].split(":");
					if (g.length > 2)
						g[1] = jQuery.map(g, function(l, o) {
									if (o > 0)
										return l
								}).join(":");
					if (e) {
	...
	c.fn.fmatter.rowactions = function(a, b, d, f) {
		...
		c.fmatter.isUndefined(f.formatoptions)
				|| (e = c.extend(e, f.formatoptions));
		f = function(g) {
			e.afterSave && e.afterSave(g);
			c(
					"tr#" + a + " div.ui-inline-edit, tr#" + a
							+ " div.ui-inline-del", "#" + b).show();
			c(
					"tr#" + a + " div.ui-inline-save, tr#" + a
							+ " div.ui-inline-cancel", "#" + b).hide()
		};
		var i = function(g) {
		...
		switch (d) {
			case "edit" :
				c("#" + b).jqGrid("editRow", a, e.keys, e.onEdit, e.onSuccess,
						e.url, e.extraparam, f, e.onError, i);
				c(
						"tr#" + a + " div.ui-inline-edit, tr#" + a
								+ " div.ui-inline-del", "#" + b).hide();
				c(
						"tr#" + a + " div.ui-inline-save, tr#" + a
								+ " div.ui-inline-cancel", "#" + b).show();
				break;
			case "save" :
				c("#" + b).jqGrid("saveRow", a, e.onSuccess, e.url,
						e.extraparam, f, e.onError, i);
				c(
						"tr#" + a + " div.ui-inline-edit, tr#" + a
								+ " div.ui-inline-del", "#" + b).show();
				c(
	...
	c.fn.fmatter.actions = function(a, b) {
		...
		if (typeof d == "undefined" || c.fmatter.isEmpty(d))
			return "";
		if (a.editbutton) {
			e = "onclick=$.fn.fmatter.rowactions('" + d + "','" + b.gid
					+ "','edit'," + b.pos + ");";
			f = f
	...
	c.unformat = function(a, b, d, f) {
		var e, i = b.colModel.formatter, g = b.colModel.formatoptions || {}, h = /([\.\*\_\'\(\)\{\}\+\?\\])/g, j = b.colModel.unformat
				|| c.fn.fmatter[i] && c.fn.fmatter[i].unformat;
		if (typeof j !== "undefined" && c.isFunction(j))
			...
			switch (i) {
				...
				case "number" :
					...
					break;
				case "currency" :
					g = c.extend({}, e.currency, g);
					b = g.thousandsSeparator.replace(h, "\\$1");
					b = new RegExp(b, "g");
					e = c(a).text().replace(b, "").replace(g.decimalSeparator,
							".").replace(g.prefix, "").replace(g.suffix, "");
					break;
				case "checkbox" :
					g = b.colModel.editoptions ? b.colModel.editoptions.value
							.split(":") : ["Yes", "No"];
		...
		return e ? e : f === true ? c(a).text() : c.jgrid.htmlDecode(c(a)
				.html())
	};
	c.unformat.select = function(a, b, d, f) {
		...
		if (b.value) {
			...
			if (c.fmatter.isString(e))
				for (var g = e.split(";"), h = 0, j = 0; j < g.length; j++) {
					i = g[j].split(":");
					if (i.length > 2)
						i[1] = jQuery.map(i, function(k, l) {
									if (l > 0)
										return k
								}).join(":");
					if (b) {
			...
			return this.each(function() {
						...
						if (m.grid) {
							...
							for (var q = 0; q < m.p.colModel.length; q++)
								if (m.p.colModel[q].index == f
										|| m.p.colModel[q].name == f) {
									t = q;
									break
								}
							if (t != -1) {
						...
						if (this.grid) {
							...
							if (f == "hidden") {
								...
								a(".ui-jqgrid-hdiv, .ui-jqgrid-bdiv",
										"#gview_" + d.p.id).slideDown("fast");
								d.p.pager && a(d.p.pager).slideDown("fast");
								d.p.toppager
										&& a(d.p.toppager).slideDown("fast");
								if (d.p.toolbar[0] === true) {
									d.p.toolbar[1] == "both"
											&& a(d.grid.ubDiv)
													.slideDown("fast");
		...
		filterGrid : function(f, d) {
				...
				if (this.p.filterModel.length === 0
					...
					if (m) {
						if (this.p.gridModel === true) {
							...
							a.each(m, function(g) {
										...
										this.search = this.search === false
												...
												: true;
										q = this.editrules
												&& this.editrules.searchhidden === true
												? true
												: this.hidden === true
														? false
														: true;
										if (this.search === true && q === true) {
											e.label = b.p.gridNames === true
													? t.p.colNames[g]
													: "";
											e.name = this.name;
						...
						var h = function() {
							...
							i.p.searchdata = {};
							a.isFunction(b.p.beforeSearch)
									&& b.p.beforeSearch();
							a.each(b.p.filterModel, function() {
								j = this.index;
								switch (this.stype) {
									case "select" :
										if (n = a("select[name=" + j + "]", b)
												.val()) {
											g[j] = n;
											b.p.marksearched
													&& a("#jqgh_" + this.name,
			...
			(function	() {
							...
							a.each(b.p.filterModel, function(p) {
								...
								a(i).append("<label for='" + this.name + "'>"
										+ this.label + "</label>");
								...
								switch (this.stype) {
									...
									case "text" :
										...
										a(j).append("<input type='text' name='"
												+ (this.index || this.name)
												+ "' id='sg_" + this.name
												+ "' value='" + s + "'/>");
										l.sopt && a("input", j).attr(l.sopt);
										if (b.p.gridToolbar === true && l.width)
											a.browser.msie ? a("input", j)
													.width(l.width - 4) : a(
													"input", j).width(l.width
													- 2);
										b.p.autosearch === true
												&& a("input", j).keypress(
														function(w) {
								...
								if (b.p.formtype == "horizontal") {
									b.p.gridToolbar === true
											&& b.p.gridNames === false ? a(g)
											.append(j) : a(g).append(i)
											.append(j);
									a(g).append(j)
								} else {
			...
			return this.each(function() {
				function d(h, k) {
					...
					o[0] && jQuery.each(k, function() {
								this.data !== undefined ? o.bind(this.type,
										this.data, this.fn) : o.bind(this.type,
										this.fn)
							})
				}
						...
						switch (this.stype) {
							case "select" :
								if (g = this.surl || c.dataUrl)
									...
									var e;
									if (h.searchoptions
											&& h.searchoptions.value)
										e = h.searchoptions.value;
									else if (h.editoptions
											&& h.editoptions.value)
							...
							case "text" :
								...
								a(k)
										.append("<input type='text' style='width:95%;padding:0px;' name='"
												+ (h.index || h.name)
												+ "' id='gs_"
												+ h.name
												+ "' value='" + g + "'/>");
				...
				this.clearToolbar = function(h) {
					...
					a.each(b.p.colModel, function() {
								...
								switch (this.stype) {
									case "select" :
										var l;
										a("select[name=" + g + "] option",
												b.grid.hDiv).each(function(u) {
													if (u === 0)
														this.selected = true;
													if (a(this).text() == o) {
														this.selected = true;
...
(function(a) {
	a.extend(a.jgrid, {
		...
		createModal : function(b, c, d, f, g, h) {
			var e = document.createElement("div"), i, n = this;
			...
			j.id = b.modalhead;
			a(j).append("<span class='ui-jqdialog-title'>" + d.caption
					+ "</span>");
			var k = a("<a href='javascript:void(0)' class='ui-jqdialog-titlebar-close ui-corner-all'></a>")
					.hover(function() {
			...
			a(m).addClass("ui-jqdialog-content ui-widget-content").attr("id",
					b.modalcontent);
			a(m).append(c);
			e.appendChild(m);
			a(e).prepend(j);
			...
			a("a.ui-jqdialog-titlebar-close", j).click(function() {
				var l = a("#" + b.themodal).data("onClose") || d.onClose, o = a("#"
			...
			if (i && c.left && !h) {
				f = a(d.gbox).width()
						- (!isNaN(d.width) ? parseInt(d.width, 10) : 0) - 8;
			...
			if (d.resize)
				if (a.fn.jqResize) {
					...
					a("#" + b.themodal).jqResize(".jqResize",
							b.scrollelm ? "#" + b.scrollelm : false)
				} else
					try {
						a(e).resizable({
							handles : "se, sw",
		...
		info_dialog : function(b, c, d, f) {
			...
			i += "<div id='infocnt' style='margin:0px;padding-bottom:1em;width:100%;overflow:auto;position:relative;height:"
					+ f
					+ ";"
					+ ("text-align:" + g.align + ";")
					+ "'>"
			...
			function h(l, o) {
				...
				if (o.dataEvents) {
					a.each(o.dataEvents, function() {
								this.data !== undefined ? a(l).bind(this.type,
										this.data, this.fn) : a(l).bind(
										this.type, this.fn)
							});
					delete o.dataEvents
				}
			...
			switch (b) {
				...
				case "checkbox" :
					...
					if (c.value) {
						...
						i = d.toLowerCase();
						if (i.search(/(false|0|no|off|undefined)/i) < 0
								&& i !== "") {
							e.checked = true;
							e.defaultChecked = true;
							e.value = d
					...
					if (typeof c.dataUrl != "undefined")
						...
						if (typeof c.value === "string") {
							...
							for (i = 0; i < f.length; i++) {
								...
								b.innerHTML = g[1];
								if (!j
										&& (a.trim(g[0]) == a.trim(d) || a
												.trim(g[1]) == a.trim(d)))
									b.selected = "selected";
								if (j
										&& (a.inArray(a.trim(g[1]), k) > -1 || a
												.inArray(a.trim(g[0]), k) > -1))
									b.selected = "selected";
								e.appendChild(b)
							}
							...
							for (i in f)
								if (f.hasOwnProperty(i)) {
									...
									if (!j
										...
										b.selected = "selected";
									if (j
											&& (a.inArray(a.trim(f[i]), k) > -1 || a
													.inArray(a.trim(i), k) > -1))
										b.selected = "selected";
									e.appendChild(b)
								}
						}
				...
				case "custom" :
					...
					try {
						...
						p == "e1"
								&& this.info_dialog(a.jgrid.errors.errcap,
										"function 'custom_element' "
												+ a.jgrid.edit.msg.nodefined,
										a.jgrid.edit.bClose);
						p == "e2" ? this.info_dialog(a.jgrid.errors.errcap,
								"function 'custom_element' "
										+ a.jgrid.edit.msg.novalue,
								a.jgrid.edit.bClose) : this.info_dialog(
								a.jgrid.errors.errcap, typeof p === "string"
										? p
										: p.message, a.jgrid.edit.bClose)
					}
					break
			}
			return e
		...
		checkDate : function(b, c) {
			...
			if (i === -1)
				...
				j = f[b[i]].toString();
				if (j.length < 1 || f[b[i]] < 1 || f[b[i]] > 12)
					return false
			}
			if (e === -1)
				return false;
				...
				j = f[b[e]].toString();
				if (j.length < 1 || f[b[e]] < 1 || f[b[e]] > 31 || f[b[i]] == 2
						&& f[b[e]] > d(f[b[g]]) || f[b[e]] > n[f[b[i]]])
					return false
			}
			return true
		},
		...
		checkValues : function(b, c, rowid,d) {
			var f, g, h, e;
			if (typeof c == "string") {
				g = 0;
				for (e = d.p.colModel.length; g < e; g++)
					if (d.p.colModel[g].name == c) {
				...
				f = d.p.colModel[c].editrules;
			if (f) {
				h || (h = d.p.colNames[c]);
				if (f.required === true)
					if (this.isEmpty(b))
						return [false, h + ": " + a.jgrid.edit.msg.required, ""];
				g = f.required === false ? false : true;
				if (f.number === true)
					if (!(g === false && this.isEmpty(b)))
						if (isNaN(b))
							return [false, h + ": " + a.jgrid.edit.msg.number,
									""];
				if (typeof f.minValue != "undefined" && !isNaN(f.minValue))
					if (parseFloat(b) < parseFloat(f.minValue))
						return [
								false,
								h + ": " + a.jgrid.edit.msg.minValue + " "
										+ f.minValue, ""];
				if (typeof f.maxValue != "undefined" && !isNaN(f.maxValue))
					if (parseFloat(b) > parseFloat(f.maxValue))
						return [
								false,
								h + ": " + a.jgrid.edit.msg.maxValue + " "
										+ f.maxValue, ""];
				if (f.email === true)
					if (!(g === false && this.isEmpty(b))) {
						...
						if (!e.test(b))
							return [false, h + ": " + a.jgrid.edit.msg.email,
									""]
					}
				if (f.integer === true)
					if (!(g === false && this.isEmpty(b))) {
						if (isNaN(b))
							return [false, h + ": " + a.jgrid.edit.msg.integer,
									""];
				...
				if (f.date === true)
					if (!(g === false && this.isEmpty(b))) {
						c = d.p.colModel[c].formatoptions
								...
								? d.p.colModel[c].formatoptions.newformat
								: d.p.colModel[c].datefmt || "Y-m-d";
						if (!this.checkDate(c, b))
							return [
									false,
									h + ": " + a.jgrid.edit.msg.date + " - "
											+ c, ""]
					}
				if (f.time === true)
					if (!(g === false && this.isEmpty(b)))
				...
				if (f.url === true)
					if (!(g === false && this.isEmpty(b))) {
						...
						if (!e.test(b))
							return [false, h + ": " + a.jgrid.edit.msg.url, ""]
					}
				if (f.custom === true)
					if (!(g === false && this.isEmpty(b)))
						if (a.isFunction(f.custom_func)) {
							b = f.custom_func.call(d, b, h,rowid);
							return a.isArray(b) ? b : [false,
	...
	a.jgrid.extend({
			...
			return this.each(function() {
				function b(l, u) {
					...
					if (u) {
						...
						if (u.rules) {
							var t, I = 0, i = u.rules.length;
							for (t = false; I < i; I++) {
								t = u.rules[I];
								if (t.field !== undefined && t.op !== undefined
										&& t.data !== undefined)
									(t = l.SearchFilter.setFilter({
												sfref : l.SearchFilter.$
														.find(".sf:last"),
												filter : a.extend({}, t)
											}))
											&& l.SearchFilter.add()
				...
				function p(l) {
					...
					l.hide();
					e.overlay === true
							&& a(".jqgrid-overlay:first", "#gbox_" + w.p.id)
									.hide()
				}
				function F() {
				...
				function y(l) {
					var u = l !== undefined, t = a("#" + w.p.id), I = {};
				...
				function z(l) {
					...
					u[0].p.search = false;
					if (e.multipleSearch === false)
						t[e.sField] = t[e.sValue] = t[e.sOper] = "";
					else
						t[e.sFilter] = "";
				...
				if (w.grid) {
					...
					if (a.fn.searchFilter) {
						...
						if (a("#" + o).html() != null) {
							...
							if (m !== false) {
								F();
								a.isFunction(e.afterShowSearch)
										&& e.afterShowSearch(a("#" + o))
							}
						} else {
							var r = [], v = a("#" + w.p.id).jqGrid(
									"getGridParam", "colNames"), d = a("#"
									+ w.p.id)
									.jqGrid("getGridParam", "colModel"), q = [
									"eq", "ne", "lt", "le", "gt", "ge", "bw",
									"bn", "in", "ni", "ew", "en", "cn", "nc"], h, j, k, E = [];
							if (e.sopt !== null)
								for (h = k = 0; h < e.sopt.length; h++) {
							...
							a.each(d, function(l, u) {
								var t = typeof u.search === "undefined"
										? true
										: u.search, I = u.hidden === true;
								l = a.extend({}, {
											text : v[l],
								...
								if (typeof l.sopt !== "undefined") {
									...
									l.ops = [];
									if (l.sopt.length > 0)
										for (h = 0; h < l.sopt.length; h++)
											if ((j = a.inArray(l.sopt[h], q)) != -1) {
												l.ops[k] = {
													op : l.sopt[h],
													text : e.odata[j]
												};
								...
								if (this.stype == "select")
									if (l.dataUrl === undefined) {
										var i;
										if (l.value)
											i = l.value;
										else if (this.editoptions)
											i = this.editoptions.value;
										if (i) {
											l.dataValues = [];
											if (typeof i === "string") {
												i = i.split(";");
							...
							if (r.length > 0) {
								...
								if (e.stringResult === undefined)
									e.stringResult = e.multipleSearch;
								...
								e.multipleSearch === true
										&& e.loadDefaults === true && b(w, e);
								a.isFunction(e.onInitializeSearch)
										&& e.onInitializeSearch(a("#" + o));
								if (a.isFunction(e.beforeShowSearch)) {
									m = e.beforeShowSearch(a("#" + o));
									if (typeof m == "undefined")
										m = true
								...
								if (m !== false) {
									...
									a.isFunction(e.afterShowSearch)
											&& e.afterShowSearch(a("#" + o));
									e.closeOnEscape === true
											&& a("#" + o).keydown(function(l) {
												l.which == 27 && p(a("#" + o));
												l.which == 13
														&& a(".ui-search", this)
																.click()
		...
		editGridRow : function(e, b) {
			...
			return this.each(function() {
						...
						if (g.length) {
							...
							a.each(d.p.colModel, function() {
								if (this.name == n
									...
									try {
										f[n] = this.editoptions.custom_value(a(
														"#" + n, "#" + j),
												"get");
										if (f[n] === undefined)
											throw "e1";
							...
							switch (a(this).get(0).type) {
								case "checkbox" :
									...
									break;
								case "select-one" :
									f[this.name] = a("option:selected", this)
											.val();
									J[this.name] = a("option:selected", this)
											.text();
									break;
								case "select-multiple" :
									f[this.name] = a(this).val();
									f[this.name] = f[this.name] ? f[this.name]
											.join(",") : "";
									var x = [];
							...
							if (d.p.autoencode)
								f[this.name] = a.jgrid.htmlEncode(f[this.name])
						}
					});
					return true
				}
				function F(g, n, x, s) {
					for (var G, B, C, L = 0, A, M, D, S = [], H = false, aa = "", Q = 1; Q <= s; Q++)
						aa += "<td class='CaptionTD'>&#160;</td><td class='DataTD'>&#160;</td>";
					if (g != "_empty")
						H = a(n).jqGrid("getInd", g);
					a(n.p.colModel).each(function(U) {
						...
						M = (B = this.editrules
								...
								: "";
						if (G !== "cb" && G !== "subgrid"
								&& this.editable === true && G !== "rn") {
							if (H === false)
								A = "";
							else if (G == n.p.ExpandColumn
									&& n.p.treeGrid === true)
								A = a("td:eq(" + U + ")", n.rows[H]).text();
							else
								try {
									A = a.unformat(a("td:eq(" + U + ")",
													n.rows[H]), {
												rowId : g,
												colModel : this
											}, U)
							...
							var W = a.extend({}, this.editoptions || {}, {
										...
										rowabove : false,
										rowcontent : ""
									}, this.formoptions || {}), ba = parseInt(
									X.rowpos, 10)
									|| L + 1, ea = parseInt((parseInt(X.colpos,
											10) || 1)
											* 2, 10);
							if (g == "_empty" && W.defaultValue)
								A = a.isFunction(W.defaultValue) ? W
										.defaultValue() : W.defaultValue;
							if (!this.edittype)
								this.edittype = "text";
							if (d.p.autoencode)
								A = a.jgrid.htmlDecode(A);
							D = a.jgrid.createEl(this.edittype, W, A, false,
									a.extend({}, a.jgrid.ajaxOptions,
											n.p.ajaxSelectOptions || {}));
							if (A === "" && this.edittype == "checkbox")
								A = a(D).attr("offval");
							if (A === "" && this.edittype == "select")
								A = a("option:eq(0)", D).text();
							...
							a("td:eq(" + (ea - 2) + ")", C[0])
									.html(typeof X.label === "undefined"
											? n.p.colNames[U]
											: X.label);
							a("td:eq(" + (ea - 1) + ")", C[0])
									.append(X.elmprefix).append(D)
									.append(X.elmsuffix);
							S[L] = U;
							L++
						}
					...
					if (L > 0) {
						Q = a("<tr class='FormData' style='display:none'><td class='CaptionTD'></td><td colspan='"
								+ (s * 2 - 1)
								+ "' class='DataTD'><input class='FormElement' id='id_g' type='text' name='"
								+ n.p.id + "_id' value='" + g + "'/></td></tr>");
						Q[0].rp = L + 999;
						a(x).append(Q);
						if (c.checkOnSubmit || c.checkOnUpdate)
					...
					if (g == "_empty") {
						a(D).each(function() {
							...
							if (C[0] != null) {
								A = "";
								if (L.defaultValue) {
									A = a.isFunction(L.defaultValue) ? L
									...
									A = a(C).attr("offval")
										...
										&& C[0].type.substr(0, 6) == "select")
									C[0].selectedIndex = 0;
								else
									C.val(A);
								if (c.checkOnSubmit === true || c.checkOnUpdate)
									c._savedData[s] = A
							}
						});
						a("#id_g", "#" + x).val(g)
						...
						if (S) {
							a("td", S).each(function(H) {
								...
								if (s !== "cb" && s !== "subgrid" && s !== "rn"
									...
									if (d.p.autoencode)
										B = a.jgrid.htmlDecode(B);
									if (c.checkOnSubmit === true
											|| c.checkOnUpdate)
										c._savedData[s] = B;
									s = a.jgrid.jqID(s);
									switch (D[H].edittype) {
										...
										case "select" :
											...
											a("#" + s + " option", "#" + x)
													.each(function() {
														this.selected = !D[H].editoptions.multiple
																&& (Q[0] == a
																		.trim(a(this)
																				.text()) || Q[0] == a
																		.trim(a(this)
										...
										case "checkbox" :
											B += "";
											if (D[H].editoptions
													&& D[H].editoptions.value)
												if (D[H].editoptions.value
														.split(":")[0] == B) {
													a("#" + s, "#" + x).attr(
															"checked", true);
													a("#" + s, "#" + x).attr(
										...
										case "custom" :
											try {
												if (D[H].editoptions
														&& a
																.isFunction(D[H].editoptions.custom_value))
													D[H].editoptions
															.custom_value(
																	a(
																			"#"
				...
				function z() {
					var g, n = [true, "", ""], x = {}, s = d.p.prmNames, G, B;
					if (a.isFunction(c.beforeCheckValues)) {
						var C = c.beforeCheckValues(f, a("#" + h), f[d.p.id
										+ "_id"] == "_empty"
										? s.addoper
										: s.editoper);
					...
					if (n[0]) {
						if (a.isFunction(c.onclickSubmit))
							x = c.onclickSubmit(c, f) || {};
						if (a.isFunction(c.beforeSubmit))
							n = c.beforeSubmit(f, a("#" + h))
					}
					if (n[0] && !c.processing) {
						c.processing = true;
						...
						B = s.oper;
						G = s.id;
						f[B] = a.trim(f[d.p.id + "_id"]) == "_empty"
								? s.addoper
								: s.editoper;
						if (f[B] != s.addoper)
							f[G] = f[d.p.id + "_id"];
						...
						x = a.extend({
							url : c.url ? c.url : a(d).jqGrid("getGridParam",
									"editurl"),
							type : c.mtype,
							data : a.isFunction(c.serializeEditData) ? c
									.serializeEditData(f) : f,
							complete : function(A, M) {
								if (M != "success") {
									n[0] = false;
									n[1] = a.isFunction(c.errorTextFormat) ? c
											.errorTextFormat(A) : M
											+ " Status: '" + A.statusText
											+ "'. Error code: " + A.status
								} else if (a.isFunction(c.afterSubmit))
									n = c.afterSubmit(A, f);
								if (n[0] === false) {
									...
									f = a.extend(f, J);
									d.p.autoencode
											&& a.each(f, function(H, aa) {
														f[H] = a.jgrid
																.htmlDecode(aa)
													});
									c.reloadAfterSubmit = c.reloadAfterSubmit
											&& d.p.datatype != "local";
									if (f[B] == s.addoper) {
										n[2]
												|| (n[2] = parseInt(
										...
										if (c.reloadAfterSubmit) {
											...
											d.p.treeGrid === true ? a(d)
													.jqGrid("setTreeRow", f[G],
															f) : a(d).jqGrid(
													"setRowData", f[G], f);
										c.closeAfterEdit
												&& a.jgrid.hideModal("#"
								...
								if (c.checkOnSubmit || c.checkOnUpdate) {
									...
									if (c._savedData[d.p.id + "_id"] != "_empty")
										for (var D in c._savedData)
											if (f[D])
												c._savedData[D] = f[D]
								}
								a("#sData", "#" + j + "_2")
						...
						if (!x.url && !c.useDataProxy)
							if (a.isFunction(d.p.dataProxy))
								c.useDataProxy = true;
							else {
								n[0] = false;
						...
						if (n[0])
							c.useDataProxy ? d.p.dataProxy.call(d, x, "set_"
											+ d.p.id) : a.ajax(x)
					}
					if (n[0] === false) {
						a("#FormError>td", "#" + j).html(n[1]);
				...
				function m() {
					if (e !== "_empty" && typeof d.p.savedRow !== "undefined"
							&& d.p.savedRow.length > 0
							&& a.isFunction(a.fn.jqGrid.restoreRow))
						for (var g = 0; g < d.p.savedRow.length; g++)
							if (d.p.savedRow[g].id == e) {
				...
				function r(g, n) {
					g === 0 ? a("#pData", "#" + j + "_2")
							.addClass("ui-state-disabled") : a("#pData",
							"#" + j + "_2").removeClass("ui-state-disabled");
					g == n ? a("#nData", "#" + j + "_2")
							.addClass("ui-state-disabled") : a("#nData",
							"#" + j + "_2").removeClass("ui-state-disabled")
				}
				function v() {
					var g = a(d).jqGrid("getDataIDs"), n = a("#id_g", "#" + j)
							.val();
					return [a.inArray(n, g), g]
				}
				var d = this;
				if (d.grid && e) {
					var q = d.p.id, h = "FrmGrid_" + q, j = "TblGrid_" + q, k = {
						themodal : "editmod" + q,
						modalhead : "edithd" + q,
						modalcontent : "editcnt" + q,
						scrollelm : h
					}, E = a.isFunction(c.beforeShowForm)
							? c.beforeShowForm
							: false, P = a.isFunction(c.afterShowForm)
							? c.afterShowForm
							: false, l = a.isFunction(c.beforeInitData)
							? c.beforeInitData
							: false, u = a.isFunction(c.onInitializeForm)
							? c.onInitializeForm
							: false, t = true, I = 1, i = 0, f, J, R, O;
					if (e == "new") {
						e = "_empty";
					...
					if (a("#" + k.themodal).html() != null) {
						...
						if (t === false)
							return;
						m();
						a(".ui-jqdialog-title", "#" + k.modalhead)
								.html(b.caption);
						a("#FormError", "#" + j).hide();
						if (c.topinfo) {
						...
						if (c.bottominfo) {
							...
							a(".binfo", "#" + j + "_2").hide();
						y(e, d, h);
						e == "_empty" || !c.viewPagerButtons ? a(
								"#pData, #nData", "#" + j + "_2").hide() : a(
								"#pData, #nData", "#" + j + "_2").show();
						...
						a(K).append(N);
						l = a("<tr id='FormError' style='display:none'><td class='ui-state-error' colspan='"
								+ I * 2 + "'></td></tr>");
						l[0].rp = 0;
						a(N).append(l);
						l = a("<tr style='display:none' class='tinfo'><td class='topinfo' colspan='"
								+ I * 2 + "'>" + c.topinfo + "</td></tr>");
						l[0].rp = 0;
						a(N).append(l);
						t = (l = d.p.direction == "rtl" ? true : false)
								? "nData"
								: "pData";
						var V = l ? "pData" : "nData";
						F(e, d, N, I);
						...
						var Z = "<a href='javascript:void(0)' id='sData' class='fm-button ui-state-default ui-corner-all'>"
								...
								+ b.bCancel + "</a>";
						t = "<table border='0' cellspacing='0' cellpadding='0' class='EditTable' id='"
								+ j
								...
								+ $
								+ "</td></tr>";
						t += "<tr style='display:none' class='binfo'><td class='bottominfo' colspan='2'>"
								+ c.bottominfo + "</td></tr>";
						t += "</tbody></table>";
						if (i > 0) {
							var Y = [];
						...
						a.jgrid.createModal(k, K, b, "#gview_" + d.p.id,
								a("#gbox_" + d.p.id)[0]);
						if (l) {
							a("#pData, #nData", "#" + j + "_2").css("float",
									"right");
							a(".EditButton", "#" + j + "_2").css("text-align",
									"left")
						}
						...
						if (b.checkOnUpdate) {
							a("a.ui-jqdialog-titlebar-close span",
									"#" + k.themodal).removeClass("jqmClose");
							a("a.ui-jqdialog-titlebar-close", "#" + k.themodal)
						...
						if (c.checkOnSubmit || c.checkOnUpdate) {
							...
							a("<div class='ui-widget-overlay jqgrid-overlay confirm' style='z-index:"
									+ K
									+ ";display:none;'>&#160;"
									+ (a.browser.msie && a.browser.version == 6
											? '<iframe style="display:block;position:absolute;z-index:-1;filter:Alpha(Opacity=\'0\');" src="javascript:false;"></iframe>'
											: "")
						...
						a("#sData", "#" + j + "_2").click(function() {
									...
									a("#FormError", "#" + j).hide();
									p();
									if (f[d.p.id + "_id"] == "_empty")
										z();
									else if (b.checkOnSubmit === true) {
										R = a.extend({}, f, J);
										if (O = w(R, c._savedData)) {
											a("#" + h).data("disabled", true);
											a(".confirm", "#" + k.themodal)
													.show()
										} else
											z()
									} else
		...
		viewGridRow : function(e, b) {
			...
			return this.each(function() {
				...
				function F(i, f, J, R) {
					for (var O, T, K, N = 0, V, Z, $ = [], Y = false, ca = "<td class='CaptionTD form-view-label ui-widget-content' width='"
							+ b.labelswidth
							+ "'>&#160;</td><td class='DataTD form-view-data ui-helper-reset ui-widget-content'>&#160;</td>", g = "", n = [
							"integer", "number", "currency"], x = 0, s = 0, G, B, C, L = 1; L <= R; L++)
						g += L == 1
								? ca
					...
					a(f.p.colModel).each(function() {
						T = this.editrules
								...
								? false
								: this.hidden === true ? true : false;
						if (!T && this.align === "right")
							if (this.formatter
									&& a.inArray(this.formatter, n) !== -1)
								x = Math.max(x, parseInt(this.width, 10));
							else
								s = Math.max(s, parseInt(this.width, 10))
					});
					G = x !== 0 ? x : s !== 0 ? s : 0;
					...
					a(f.p.colModel).each(function(A) {
						...
						Z = (T = this.editrules
								...
								? "style='display:none'"
								: "";
						C = typeof this.viewable != "boolean"
								? true
								: this.viewable;
						if (O !== "cb" && O !== "subgrid" && O !== "rn" && C) {
							V = Y === false ? "" : O == f.p.ExpandColumn
									&& f.p.treeGrid === true ? a(
									"td:eq(" + A + ")", f.rows[Y]).text() : a(
									"td:eq(" + A + ")", f.rows[Y]).html();
							B = this.align === "right" && G !== 0
									? true
									: false;
							...
							var M = a.extend({}, {
										rowabove : false,
										rowcontent : ""
									}, this.formoptions || {}), D = parseInt(
									M.rowpos, 10)
									|| N + 1, S = parseInt((parseInt(M.colpos,
											10) || 1)
											* 2, 10);
							if (M.rowabove) {
							...
							a("td:eq(" + (S - 2) + ")", K[0]).html("<b>"
									+ (typeof M.label === "undefined"
											? f.p.colNames[A]
											: M.label) + "</b>");
							a("td:eq(" + (S - 1) + ")", K[0]).append("<span>"
									+ V + "</span>").attr("id", "v_" + O);
				...
				function y(i, f) {
					...
					if (K = a(f).jqGrid("getInd", i, true)) {
						a("td", K).each(function(N) {
							J = f.p.colModel[N].name;
							R = f.p.colModel[N].editrules
									&& f.p.colModel[N].editrules.edithidden === true
									? false
									: f.p.colModel[N].hidden === true
											? true
											: false;
							if (J !== "cb" && J !== "subgrid" && J !== "rn") {
								T = J == f.p.ExpandColumn
										&& f.p.treeGrid === true ? a(this)
										.text() : a(this).html();
								a.extend({}, f.p.colModel[N].editoptions || {});
								J = a.jgrid.jqID("v_" + J);
								a("#" + J + " span", "#" + v).html(T);
								R
										&& a("#" + J, "#" + v)
				...
				function z(i, f) {
					i === 0 ? a("#pData", "#" + v + "_2")
							.addClass("ui-state-disabled") : a("#pData",
							"#" + v + "_2").removeClass("ui-state-disabled");
					i == f ? a("#nData", "#" + v + "_2")
							.addClass("ui-state-disabled") : a("#nData",
							"#" + v + "_2").removeClass("ui-state-disabled")
				}
				function w() {
					var i = a(o).jqGrid("getDataIDs"), f = a("#id_g", "#" + v)
							.val();
				...
				if (o.grid && e) {
					if (!b.imgpath)
						b.imgpath = o.p.imgpath;
					var m = o.p.id, r = "ViewGrid_" + m, v = "ViewTbl_" + m, d = {
						themodal : "viewmod" + m,
						modalhead : "viewhd" + m,
						modalcontent : "viewcnt" + m,
						scrollelm : r
					}, q = a.isFunction(b.beforeInitData)
							? b.beforeInitData
							: false, h = true, j = 1, k = 0;
					if (a("#" + d.themodal).html() != null) {
						...
						if (h === false)
							return;
						a(".ui-jqdialog-title", "#" + d.modalhead)
								.html(b.caption);
						a("#FormError", "#" + v).hide();
						y(e, o);
						a.isFunction(b.beforeShowForm)
								&& b.beforeShowForm(a("#" + r));
						...
						var E = isNaN(b.dataheight)
								? b.dataheight
								: b.dataheight + "px";
						E = a("<form name='FormPost' id='"
								+ r
						...
						E = a("<span></span>")
								...
								.append("<table border='0' class='EditTable' id='"
										...
										+ u
										+ "</td></tr></tbody></table>");
						a.jgrid.createModal(d, E, b, "#gview_" + o.p.id,
								a("#gview_" + o.p.id)[0]);
						if (q) {
							a("#pData, #nData", "#" + v + "_2").css("float",
									"right");
							a(".EditButton", "#" + v + "_2").css("text-align",
									"left")
						}
						b.viewPagerButtons
						...
						a(".fm-button:not(.ui-state-disabled)", "#" + v + "_2")
								.hover(function() {
											a(this).addClass("ui-state-hover")
										}, function() {
											a(this)
		...
		delGridRow : function(e, b) {
			...
			return this.each(function() {
				...
				if (p.grid)
					if (e) {
						var F = typeof b.beforeShowForm === "function"
								? true
								: false, y = typeof b.afterShowForm === "function"
								? true
								: false, z = a.isFunction(b.beforeInitData)
								? b.beforeInitData
								: false, w = p.p.id, o = {}, m = true, r = "DelTbl_"
								+ w, v, d, q, h, j = {
							themodal : "delmod" + w,
							modalhead : "delhd" + w,
						...
						if (a("#" + j.themodal).html() != null) {
							...
							z += "<tr id='DelData' style='display:none'><td >"
									+ e + "</td></tr>";
							z += '<tr><td class="delmsg" style="white-space:pre;">'
									+ b.msg
									+ "</td></tr><tr><td >&#160;</td></tr>";
							z += "</tbody></table></div>";
							z += "<table cellspacing='0' cellpadding='0' border='0' class='EditTable' id='"
									+ r
									...
									+ "</td></tr></tbody></table>";
							b.gbox = "#gbox_" + w;
							a.jgrid.createModal(j, z, b, "#gview_" + p.p.id,
									a("#gview_" + p.p.id)[0]);
							...
							b.delicon = a.extend([true, "left",
											"ui-icon-scissors"], b.delicon);
							b.cancelicon = a.extend([true, "left",
											"ui-icon-cancel"], b.cancelicon);
							if (b.delicon[0] === true)
								a("#dData", "#" + r + "_2")
										.addClass(b.delicon[1] == "right"
												? "fm-button-icon-right"
												: "fm-button-icon-left")
										.append("<span class='ui-icon "
												+ b.delicon[2] + "'></span>");
							...
							a("#dData", "#" + r + "_2").click(function() {
								...
								if (k[0] && !c.processing) {
									...
									q = p.p.prmNames;
									v = a.extend({}, c.delData, o);
									h = q.oper;
									v[h] = q.deloper;
									d = q.id;
									v[d] = E;
									var P = a.extend({
										url : c.url ? c.url : a(p).jqGrid(
												"getGridParam", "editurl"),
										type : b.mtype,
										data : a.isFunction(b.serializeDelData)
												? b.serializeDelData(v)
												: v,
										complete : function(l, u) {
											if (u != "success") {
												k[0] = false;
												k[1] = a
														.isFunction(c.errorTextFormat)
														? c.errorTextFormat(l)
														: u
																+ " Status: '"
																+ l.statusText
																+ "'. Error code: "
																+ l.status
											} else if (typeof c.afterSubmit === "function")
												k = c.afterSubmit(l, v);
											if (k[0] === false) {
												a("#DelError>td", "#" + r)
														.html(k[1]);
									...
									if (!P.url && !c.useDataProxy)
										if (a.isFunction(p.p.dataProxy))
											c.useDataProxy = true;
										else {
											k[0] = false;
									...
									if (k[0])
										c.useDataProxy ? p.p.dataProxy.call(p,
												P, "del_" + p.p.id) : a.ajax(P)
								}
								if (k[0] === false) {
		...
		navGrid : function(e, b, p, F, y, z, w) {
			...
			return this.each(function() {
				...
				if (!(!m.grid || typeof e != "string")) {
					...
					r = 1;
					if (b.cloneToTop && m.p.toppager)
						r = 2;
					for (v = 0; v < r; v++) {
						var q = a("<table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table navtable' style='float:left;table-layout:auto;'><tbody><tr></tr></tbody></table>"), h, j;
						if (v === 0) {
							h = e;
							j = m.p.id;
							if (h == m.p.toppager) {
						...
						if (b.edit) {
							...
							a(d, q).attr({
								...
								if (!a(this).hasClass("ui-state-disabled")) {
									var k = m.p.selrow;
									if (k)
										typeof b.editfunc == "function" ? b
												.editfunc(k) : a(m).jqGrid(
												"editGridRow", k, p);
									else {
						...
						if (b.refresh) {
							...
							a(d, q).attr({
								...
								if (!a(this).hasClass("ui-state-disabled")) {
									...
									m.p.search = false;
									try {
										a("#fbox_" + m.p.id).searchFilter()
												.reset({
															reload : false
														});
										a.isFunction(m.clearToolbar)
						...
						if (b.print) {
							d = a("<td class='ui-pg-button ui-corner-all'></td>");
							a(d).append("<div class='ui-pg-div'><span class='ui-icon "
										+ b.printicon + "'></span>" + b.refreshtext + "</div>");
							a("tr", q).append(d);
							a(d, q).attr({
										title : b.printtitle || "",
										id : "print_" + j
								...
								var _prop = a(m)[0].p;
								var cols = "", colNames = "";
                for (var i = 0; i < _prop.colModel.length; i++) {
                  var _colName = _prop.colModel[i].name;
                  if(_prop.colModel[i].hidden == false){
                  	if (_colName !== "cb" && _colName !== "subgrid"
	                      && _colName !== "rn") {
	                    cols += _colName + ',';
	                  }
	                  var _col = _prop.colNames[i];
	                  if (_col !== "" && _col !== " "
	                      && _col.indexOf("checkbox")==-1){
	                    colNames += _col + ',';
	                  } 
                  }
                ...
                colNames = colNames.substring(0,colNames.length-1);
								var rowNo = 0;
						    var dataIDs = jQuery("#mainGrid").getDataIDs();
						    var postData = "{'rowTotal':'" + dataIDs.length + "',";
						    if (dataIDs) {
						      $.each(dataIDs, function(id, rowId) {
						            rowNo = rowNo + 1;
						            postData = postData + "'" + rowNo + "':{";
						            var rowData = jQuery("#mainGrid").getRowData(rowId);
                          ...
                          //str = str.replace(/\n[\s| | ]*\r/g,'\n'); //去除多余空行
						    ...
						    if (formExportExcel.length == 0) {
							    	if (!_prop.exportUrl) {
		                  var last = _prop.url.indexOf("!");
		                  if (last != -1) {
		                    _prop.exportUrl = _prop.url.substring(0, last) + "!export.action"; 
		                  } else {
		                    last = _prop.url.indexOf(".action");
		                    _prop.exportUrl = _prop.url.substring(0, last) + "!export.action";
		                  }
		                }
						    	
                    formExportExcel = $("<form id='formExportExcel'><input id='jQGridData' name='jQGridData' type='hidden'><input id='cols' name='cols' type='hidden'><input id='colNames' name='colNames' type='hidden'></form>")
                        .attr("action", _prop.exportUrl).attr("target", "fmExport").attr("method", "POST")
                        .appendTo($("body"));
                }
                
                $("#jQGridData", formExportExcel).val(postData);
                $("#cols", formExportExcel).val(cols);
                $("#colNames", formExportExcel).val(colNames);
						    formExportExcel.submit();
					
                //window.location=_prop.exportUrl + "?jQGridData=" + postData + "&cols=" + cols + "&colNames=" + encodeURI(colNames);   
							}).hover(function() {
								a(this).hasClass("ui-state-disabled")
										|| a(this).addClass("ui-state-hover")
							}, function() {
		...
		navButtonAdd : function(e, b) {
			...
			return this.each(function() {
				if (this.grid) {
					if (e.indexOf("#") !== 0)
						e = "#" + e;
					var p = a(".navtable", e)[0], F = this;
					if (p) {
						var y = a("<td></td>");
						b.buttonicon.toString().toUpperCase() == "NONE"
								? a(y).addClass("ui-pg-button ui-corner-all")
										.append("<div class='ui-pg-div'>"
												+ b.caption + "</div>")
								: a(y)
						...
						if (b.position == "first")
							p.rows[0].cells.length === 0
									? a("tr", p).append(y)
									: a("tr td:eq(0)", p).before(y);
						else
							a("tr", p).append(y);
						a(y, p).attr("title", b.title || "").click(function(z) {
							a(this).hasClass("ui-state-disabled")
									|| a.isFunction(b.onClickButton)
									&& b.onClickButton.call(F, z);
							return false
						}).hover(function() {
							a(this).hasClass("ui-state-disabled")
									|| a(this).addClass("ui-state-hover")
		...
		navSeparatorAdd : function(e, b) {
			...
			return this.each(function() {
				if (this.grid) {
					...
					if (p) {
						var F = "<td class='ui-pg-button ui-state-disabled' style='width:4px;'><span class='"
								+ b.sepclass
								+ "'></span>"
								+ b.sepcontent
								+ "</td>";
		...
		FormToGrid : function(e, b, p, F) {
			return this.each(function() {
						...
						if (y.grid) {
							p || (p = "set");
							F || (F = "first");
							var z = a(b).serializeArray(), w = {};
							a.each(z, function(o, m) {
										w[m.name] = m.value
									});
							if (p == "add")
								a(y).jqGrid("addRowData", e, w, F);
							else
								p == "set" && a(y).jqGrid("setRowData", e, w)
						}
					})
		}
	})
...
jQuery.fn.searchFilter = function(k, H) {
	function I(e, l, v) {
		this.$ = e;
		this.add = function(a) {
			a == null ? e.find(".ui-add-last").click() : e.find(".sf:eq(" + a
					+ ") .ui-add").click();
			return this
		};
		this.del = function(a) {
			a == null ? e.find(".sf:last .ui-del").click() : e.find(".sf:eq("
					+ a + ") .ui-del").click();
			return this
		};
		...
		if (l != null) {
			...
			function F(a, b) {
				...
				d[0] != null && jQuery.each(b, function() {
							this.data != null ? d.bind(this.type, this.data,
									this.fn) : d.bind(this.type, this.fn)
						})
			}
			...
			var x = e.find("tr.sf"), G = x.find("td.fields"), y = x
					.find("td.ops"), o = x.find("td.data"), s = "";
			...
			y.append(s);
			o
					.append("<input type='text' class='default' style='display:none;' />");
			...
			jQuery.each(l, function(a) {
				...
				this.dataInit != null && a != n
						&& E(".field" + a, this.dataInit);
				this.dataEvents != null && a != n
						&& F(".field" + a, this.dataEvents)
			});
			...
			e.find(".ui-search").click(function() {
				var a = jQuery(e.selector), b, d = a
						.find("select[name='groupOp'] :selected").val();
				...
				a.find(".sf").each(function(c) {
					var g = jQuery(this).find("select[name='field'] :selected")
							.val(), i = jQuery(this)
							.find("select[name='op'] :selected").val(), h = jQuery(this)
			...
			this.setFilter = function(a) {
				...
				for (i = d.length; c < i; c++) {
					if (selDOMobj = b.find(".ops > select[class='field" + c
							+ "']")[0]) {
						g = 0;
						for (h = selDOMobj.options.length; g < h; g++)
							j[d[c]].ops[selDOMobj.options[g].value] = g
					}
					if (selDOMobj = b.find(".data > select[class='field" + c
							+ "']")[0]) {
						j[d[c]].data = {};
						g = 0;
						for (h = selDOMobj.options.length; g < h; g++)
							...
							a("td", h).each(function(j) {
								...
								if (k != "cb" && k != "subgrid" && k != "rn") {
									...
									if (g[j].editable === true) {
										...
										if (!g[j].edittype)
											g[j].edittype = "text";
										e = a.jgrid.createEl(g[j].edittype, e,
												l, true, a.extend({},
														a.jgrid.ajaxOptions,
														b.p.ajaxSelectOptions
																|| {}));
						...
						if (l === "1") {
							...
							a("td", q).each(function(m) {
								...
								if (f != "cb" && f != "subgrid"
									...
									p = a.jgrid.checkValues(b[f], m, d,c);
									if (p[0] === false) {
										p[1] = b[f] + " " + p[1];
										return false
									}
							...
							if (p[0] === false)
								try {
									var g = a.jgrid.findPos(a("#"
													+ a.jgrid.jqID(d),
											c.grid.bDiv)[0]);
									a.jgrid.info_dialog(a.jgrid.errors.errcap,
											p[1], a.jgrid.edit.bClose, {
												left : g[0],
												top : g[1]
											})
								} catch (j) {
								...
								if (i == "clientArray") {
									...
									a.ajax(a.extend({
										url : i,
										data : a
												.isFunction(c.p.serializeRowData)
												? c.p.serializeRowData.call(c,
														b)
												: b,
										type : "POST",
					...
					if (c.p.savedRow.length > 0) {
						if (a === true)
							if (d == c.p.iRow && e == c.p.iCol)
								return;
						b(c).jqGrid("saveCell", c.p.savedRow[0].id,
								c.p.savedRow[0].ic)
					} else
						window.setTimeout(function() {
									b("#" + c.p.knv).attr("tabindex", "-1")
											.focus()
								}, 0);
					...
					if (!(h == "subgrid" || h == "cb" || h == "rn")) {
						...
						if (c.p.colModel[e].editable === true && a === true
							...
							if (parseInt(c.p.iCol, 10) >= 0
								...
								b("td:eq(" + c.p.iCol + ")", c.rows[c.p.iRow])
										.removeClass("edit-cell ui-state-highlight");
								b(c.rows[c.p.iRow])
										.removeClass("selected-row ui-state-hover")
							}
							b(g).addClass("edit-cell ui-state-highlight");
							b(c.rows[d])
									.addClass("selected-row ui-state-hover");
							try {
								f = b.unformat(g, {
											rowId : c.rows[d].id,
											colModel : c.p.colModel[e]
							...
							if (b.isFunction(c.p.formatCell)) {
								var j = c.p.formatCell.call(c, c.rows[d].id, h,
										f, d, e);
								if (j !== undefined)
									f = j
							}
							...
							var i = b.jgrid.createEl(c.p.colModel[e].edittype,
									j, f, true, b.extend({},
											b.jgrid.ajaxOptions,
											c.p.ajaxSelectOptions || {}));
							b.isFunction(c.p.beforeEditCell)
									&& c.p.beforeEditCell.call(c, c.rows[d].id,
											h, f, d, e);
							...
							b.isFunction(c.p.afterEditCell)
									&& c.p.afterEditCell.call(c, c.rows[d].id,
											h, f, d, e)
						} else {
							if (parseInt(c.p.iCol, 10) >= 0
									&& parseInt(c.p.iRow, 10) >= 0) {
							...
							if (b.isFunction(c.p.onSelectCell)) {
								...
								c.p.onSelectCell.call(c, c.rows[d].id, h, f, d,
										e)
							}
						}
						c.p.iCol = e;
					...
					if (c !== null) {
						...
						switch (k.edittype) {
							...
							case "checkbox" :
								...
								g = f = b("#" + d + "_" + i, a.rows[d])
										.attr("checked") ? n[0] : n[1];
								break;
							case "password" :
							case "text" :
							case "textarea" :
							...
							case "custom" :
								try {
									if (k.editoptions
										...
										f = k.editoptions.custom_value.call(a,
												b(".customelement", h), "get");
										if (f === undefined)
											throw "e2";
										else
											g = f
									...
									q == "e1"
											&& b.jgrid
													.info_dialog(
															...
															jQuery.jgrid.edit.bClose);
									q == "e2" ? b.jgrid.info_dialog(
											jQuery.jgrid.errors.errcap,
											"function 'custom_value' "
													+ b.jgrid.edit.msg.novalue,
											jQuery.jgrid.edit.bClose) : b.jgrid
											.info_dialog(
													jQuery.jgrid.errors.errcap,
													q.message,
													jQuery.jgrid.edit.bClose)
						...
						if (g != a.p.savedRow[c].v) {
							if (b.isFunction(a.p.beforeSaveCell))
								if (c = a.p.beforeSaveCell.call(a,
										a.rows[d].id, j, f, d, e))
									g = f = c;
							var r = b.jgrid.checkValues(f, e, d,a);
							if (r[0] === true) {
								c = {};
								if (b.isFunction(a.p.beforeSubmitCell))
									(c = a.p.beforeSubmitCell.call(a,
											a.rows[d].id, j, f, d, e))
											|| (c = {});
								b("input.hasDatepicker", h).length > 0
										&& b("input.hasDatepicker", h)
												.datepicker("hide");
										...
										b.ajax(b.extend(
												{
													url : a.p.cellurl,
													data : b
															.isFunction(a.p.serializeCellData)
															? a.p.serializeCellData
																	.call(a, o)
															: o,
													type : "POST",
			...
			return this.each(function() {
						...
						if (!(!a.grid || a.p.cellEdit !== true)) {
							...
							if (c !== null) {
								...
								b(a).jqGrid("setCell", a.rows[d].id, e,
				...
				function d(g, k, j) {
					...
					if (j == "h") {
						...
						if (a.rows[g].cells[k].offsetLeft
								+ a.rows[g].cells[k].clientWidth >= j
								+ parseInt(i, 10))
						...
						switch (f) {
							case 38 :
								if (a.p.iRow - 1 > 0) {
									...
									b(a).jqGrid("editCell", a.p.iRow - 1,
											a.p.iCol, false)
								}
								break;
							case 40 :
...
(function(b) {
	...
	b.jqm = {
		hash : {},
		open : function(a, f) {
			var c = i[a], d = c.c, h = "." + d.closeClass, g = parseInt(c.w
					.css("z-index"));
			g = g > 0 ? g : 3E3;
			var j = b("<div></div>").css({
						height : "100%",
			...
			if (d.modal) {
				...
				d.closeoverlay && c.w.jqmAddClose(j);
			else
				j = e;
			...
			if (q) {
				...
				if (j) {
					...
					for (var m in {
						...
						j.style.setExpression(m.toLowerCase(),
	...
	var l = 0, i = b.jqm.hash, k = [], q = b.browser.msie
			&& b.browser.version == "6.0", e = false, r = function(a) {
		var f = b('<iframe src="javascript:false;document.write(\'\');" class="jqm"></iframe>')
				.css({
							opacity : 0
						});
		if (q)
	...
	var i = b.jqDnR, c = i.dnr, d = i.e, f, l = function(a, e, n, m) {
		return a.each(function() {
					...
					e.bind("mousedown", {
								...
								try {
									b("input.hasDatepicker", d[0])
											.datepicker("hide")
								} catch (p) {
								}
								b(document).mousemove(b.jqDnR.drag)
	...
	b.jgrid.extend({
		setSubGrid : function() {
			return this.each(function() {
						...
						e.p.colModel.unshift({
									name : "subgrid",
									width : b.browser.safari ? e.p.subGridWidth
											+ e.p.cellLayout : e.p.subGridWidth,
									sortable : false,
									resizable : false,
									hidedlg : true,
									search : false,
		...
		addSubGridCell : function(e, c) {
			...
			return n === false
					...
					: '<td role="grid" aria-describedby="' + o + '_subgrid" '
							+ a + "></td>"
		},
			...
			return this.each(function() {
				...
				if (a.grid) {
					var n = function(g, j, f) {
						j = b("<td align='" + a.p.subGridModel[0].align[f]
								+ "'></td>").html(j);
						b(g).append(j)
					}, o = function(g, j) {
									...
									b.ajax(b.extend(
											{
												...
												data : b
														.isFunction(a.p.serializeSubGridData)
					...
					b("td:eq(" + c + ")", e).click(function() {
						if (b(this).hasClass("sgcollapsed")) {
							...
							b.isFunction(a.p.subGridRowExpanded)
									? a.p.subGridRowExpanded.call(a, s + "_"
													+ m, m)
									: x(r);
							b(this)
									.html("<a href='javascript:void(0);'><span class='ui-icon ui-icon-minus'></span></a>")
							...
							b(this)
									.html("<a href='javascript:void(0);'><span class='ui-icon ui-icon-plus'></span></a>")
...
(function(f) {
	f.jgrid.extend({
		groupingSetup : function() {
			return this.each(function() {
				var c = this, b = c.p.groupingView;
				if (b !== null && (typeof b === "object" || f.isFunction(b)))
					if (b.groupField.length) {
						for (var a = 0; a < b.groupField.length; a++) {
							b.groupOrder[a] || (b.groupOrder[a] = "asc");
							b.groupText[a] || (b.groupText[a] = "{0}");
							if (typeof b.groupColumnShow[a] != "boolean")
								b.groupColumnShow[a] = true;
							if (typeof b.groupSummary[a] != "boolean")
								b.groupSummary[a] = false;
							b.groupColumnShow[a] === true ? f(c).jqGrid(
									"showCol", b.groupField[a]) : f(c).jqGrid(
									"hideCol", b.groupField[a]);
							b.sortitems[a] = [];
							b.sortnames[a] = [];
							b.summaryval[a] = [];
							if (b.groupSummary[a]) {
								b.summary[a] = [];
								for (var d = c.p.colModel, e = 0, g = d.length; e < g; e++)
									d[e].summaryType && b.summary[a].push({
												nm : d[e].name,
												st : d[e].summaryType,
												v : ""
											})
		...
		groupingPrepare : function(c, b, a, d) {
			this.each(function() {
				b[0] += "";
				var e = b[0].toString().split(" ").join(""), g = this.p.groupingView, j = this;
				...
				g.groupSummary[0] && f.each(g.summaryval[0][e], function() {
							this.v = f.isFunction(this.st) ? this.st.call(j,
									this.v, this.nm, d) : f(j).jqGrid(
									"groupingCalculations." + this.st, this.v,
									this.nm, d)
						})
			});
			return a
		...
		groupingToggle : function(c) {
			this.each(function() {
						var b = this.p.groupingView, a = c.lastIndexOf("_"), d = c
								.substring(0, a + 1);
						a = parseInt(c.substring(a + 1), 10) + 1;
						var e = b.minusicon, g = b.plusicon;
						if (f("#" + c + " span").hasClass(e)) {
							b.showSummaryOnHide && b.groupSummary[0] ? f("#"
									+ c).nextUntil(".jqfoot").hide() : f("#"
									+ c).nextUntil("#" + d + String(a)).hide();
		...
		groupingRender : function(c, b) {
			return this.each(function() {
				...
				f.each(d.sortitems[0], function(h, l) {
					j = a.p.id + "ghead_" + h;
					g = "<span style='cursor:pointer;' class='ui-icon " + m
							+ "' onclick=\"jQuery('#" + a.p.id
							+ "').jqGrid('groupingToggle','" + j
							+ "');return false;\"></span>";
					try {
						n = a
								.formatter(j, d.sortnames[0][h], q,
										d.sortitems[0])
					} catch (u) {
						n = d.sortnames[0][h]
					}
					e += '<tr id="'
							...
							+ a.p.direction + '"><td colspan="' + b + '">' + g
							+ f.jgrid.format(d.groupText[0], n, c[l].length)
							+ "</td></tr>";
										...
										if (this.nm == o[i].name) {
											...
											if (this.st == "avg")
												if (this.v && r > 0)
													this.v /= r;
											try {
												p = a.formatter("", this.v, i,
														this)
											} catch (v) {
												p = this.v
											}
											s = "<td " + a.formatCol(i, 1, "")
													+ ">"
													+ f.jgrid.format(t, p)
													+ "</td>";
											return false
		...
		groupingGroupBy : function(c, b) {
			return this.each(function() {
						...
						for (var e = 0; e < d.groupField.length; e++)
							d.groupColumnShow[e]
									|| f(a).jqGrid("showCol", d.groupField[e]);
		...
		groupingRemove : function(c) {
			return this.each(function() {
						...
						b.p.grouping = false;
						if (c === true) {
							f("tr.jqgroup, tr.jqfoot",
									"#" + b.p.id + " tbody:first").remove();
							f("tr.jqgrow:hidden", "#" + b.p.id + " tbody:first")
									.show()
						} else
							f(b).trigger("reloadGrid")
					})
		},
		...
		setTreeNode : function(b, c) {
			return this.each(function() {
				...
				if (a.grid && a.p.treeGrid) {
					var f = a.p.expColInd, e = a.p.treeReader.expanded_field, k = a.p.treeReader.leaf_field, i = a.p.treeReader.level_field, l = a.p.treeReader.loaded;
					c.level = b[i];
					if (a.p.treeGridModel == "nested") {
						var g = b[a.p.treeReader.left_field], j = b[a.p.treeReader.right_field];
						b[k]
								|| (b[k] = parseInt(j, 10) === parseInt(g, 10)
										+ 1 ? "true" : "false")
					}
					j = parseInt(b[i], 10);
					...
					g = "<div class='tree-wrap tree-wrap-" + a.p.direction
							+ "' style='width:" + g * 18 + "px;'>";
					g += "<div style='"
							+ (a.p.direction == "rtl" ? "right:" : "left:") + j
							* 18 + "px;' class='ui-icon ";
					if (b[l] != undefined)
						b[l] = b[l] == "true" || b[l] === true ? true : false;
					...
					a.p.ExpandColClick === true
							&& d("span", c).css("cursor", "pointer").bind(
									"click", function(h) {
										h = d(h.target || h.srcElement, a.rows)
												.closest("tr.jqgrow")[0].id;
										var m = a.p._index[h], n = a.p.treeReader.expanded_field;
										if (!a.p.data[m][a.p.treeReader.leaf_field])
											if (a.p.data[m][n]) {
												d(a).jqGrid("collapseRow",
														a.p.data[m]);
												d(a).jqGrid("collapseNode",
														a.p.data[m])
											} else {
												d(a).jqGrid("expandRow",
														a.p.data[m]);
												d(a).jqGrid("expandNode",
														a.p.data[m])
											}
		...
		expandRow : function(b) {
			this.each(function() {
				...
				if (c.grid && c.p.treeGrid) {
					...
					d(a).each(function() {
								var e = d.jgrid.getAccessor(this,
										c.p.localReader.id);
					...
					switch (c.p.treeGridModel) {
						case "nested" :
							...
							d(c.p.data).each(function() {
								parseInt(this[a], 10) === parseInt(
										c.p.tree_root_level, 10)
										&& b.push(this)
							});
							break;
						case "adjacency" :
			...
			this.each(function() {
						if (this.grid && this.p.treeGrid) {
							...
							switch (a.p.treeGridModel) {
								case "nested" :
									c = parseInt(b[a.p.treeReader.level_field],
											10)
		...
		getNodeParent : function(b) {
			...
			this.each(function() {
				...
				if (a.grid && a.p.treeGrid)
					switch (a.p.treeGridModel) {
						case "nested" :
							var f = a.p.treeReader.left_field, e = a.p.treeReader.right_field, k = a.p.treeReader.level_field, i = parseInt(
									b[f], 10), l = parseInt(b[e], 10), g = parseInt(
									b[k], 10);
							d(this.p.data).each(function() {
								if (parseInt(this[k], 10) === g - 1
										&& parseInt(this[f], 10) < i
										&& parseInt(this[e], 10) > l) {
									c = this;
									return false
								}
							});
							break;
						case "adjacency" :
							var j = a.p.treeReader.parent_id_field, h = a.p.localReader.id;
							d(this.p.data).each(function() {
										if (this[h] == b[j]) {
											c = this;
											return false
		...
		getFullTreeNode : function(b) {
			...
			this.each(function() {
				...
				if (a.grid && a.p.treeGrid)
					switch (a.p.treeGridModel) {
						case "nested" :
							var e = a.p.treeReader.left_field, k = a.p.treeReader.right_field, i = a.p.treeReader.level_field, l = parseInt(
									b[e], 10), g = parseInt(b[k], 10), j = parseInt(
									b[i], 10);
							d(this.p.data).each(function() {
								parseInt(this[i], 10) >= j
										&& parseInt(this[e], 10) >= l
										&& parseInt(this[e], 10) <= g
										&& c.push(this)
							});
							break;
						case "adjacency" :
							c.push(b);
							var h = a.p.treeReader.parent_id_field, m = a.p.localReader.id;
		...
		isNodeLoaded : function(b) {
			...
			this.each(function() {
						...
						if (a.grid && a.p.treeGrid) {
							...
							c = b.loaded !== undefined
									? b.loaded
									: b[f]
											|| d(a)
													.jqGrid("getNodeChildren",
															b).length > 0
											? true
											: false
						}
			...
			return this.each(function() {
						if (this.grid && this.p.treeGrid) {
							...
							for (k = j.length; e < k; e++) {
								i = j[e];
								l.push(i);
								d(this).jqGrid("collectChildrenSortTree", l, i,
										b, c, a, f)
							}
							d.each(l, function(h) {
										var m = d.jgrid.getAccessor(this,
												g.p.localReader.id);
										d("#" + g.p.id + " tbody tr:eq(" + h
												+ ")").after(d("tr#" + m,
												g.grid.bDiv))
									});
							l = j = e = null
			...
			return this.each(function() {
						if (this.grid && this.p.treeGrid) {
							...
							for (l = j.length; i < l; i++) {
								g = j[i];
								b.push(g);
								d(this).jqGrid("collectChildrenSortTree", b, g,
										a, f, e, k)
							}
						}
					})
	...
	b.jgrid.extend({
		...
		jqGridExport : function(a) {
			...
			this.each(function() {
						if (this.grid) {
							var c = b
									.extend({}, b(this).jqGrid("getGridParam"));
							if (c.rownumbers) {
							...
							switch (a.exptype) {
								case "xmlstring" :
									d = "<" + a.root + ">"
											+ xmlJsonClass.json2xml(c, a.ident)
											+ "</" + a.root + ">";
									break;
								case "jsonstring" :
									d = "{"
											+ xmlJsonClass.toJson(c, a.root,
													a.ident) + "}";
		...
		var g = function(d, c, j) {
			...
			if (d instanceof Array)
				...
				i += typeof d === "function" ? j + "<" + c + "><![CDATA[" + d
						+ "]]\></" + c + ">" : d.toString() === '""'
						|| d.toString().length === 0 ? j + "<" + c
						+ ">__EMPTY_STRING_</" + c + ">" : j + "<" + c + ">"
						+ d.toString() + "</" + c + ">";
			return i
		}, e = "", f;
		for (f in a)
	...
	b.jgrid.extend({
		setColumns : function(a) {
			...
			return this.each(function() {
				var c = this;
				if (c.grid) {
					var j = typeof a.beforeShowForm === "function"
							? true
							: false, k = typeof a.afterShowForm === "function"
							? true
							: false, l = typeof a.afterSubmitForm === "function"
							? true
					...
					if (b("#" + f.themodal).html() != null) {
						...
						for (i = 0; i < this.p.colNames.length; i++)
							c.p.colModel[i].hidedlg
									|| (g += "<tr><td style='white-space: pre;'><input type='checkbox' style='margin-right:5px;' id='col_"
											...
											+ "' class='cbox' value='T' "
											+ (this.p.colModel[i].hidden === false
													? "checked"
													: "")
											+ "/><label for='col_"
											+ this.p.colModel[i].name
						...
						g += "<table border='0' class='EditTable' id='"
								...
								+ "</td></tr></tbody></table>";
						a.gbox = "#gbox_" + e;
						b.jgrid.createModal(f, g, a, "#gview_" + c.p.id,
								b("#gview_" + c.p.id)[0]);
						if (a.saveicon[0] == true)
							b("#dData", "#" + d + "_2")
									.addClass(a.saveicon[1] == "right"
											? "fm-button-icon-right"
											: "fm-button-icon-left")
									.append("<span class='ui-icon "
											+ a.saveicon[2] + "'></span>");
		...
		if (!this.grid) {
			...
			jQuery("tbody > tr", jQuery(this)).each(function() {
				...
				jQuery("td", jQuery(this)).each(function() {
					if (e === 0 && l) {
						var k = jQuery("input", jQuery(this)), m = k
								.attr("value");
						h.push(m || f.length);
...
(function(a) {
	...
	a.jgrid.extend({
		sortableColumns : function(b) {
			return this.each(function() {
				...
				var c = this, h = {
					...
					placeholder : {
						element : function(e) {
							return a(document.createElement(e[0].nodeName))
									.addClass(e[0].className
											+ " ui-sortable-placeholder ui-state-highlight")
									.removeClass("ui-sortable-helper")[0]
						},
						update : function(e, g) {
							g.height(e.currentItem.innerHeight()
									- parseInt(e.currentItem.css("paddingTop")
													|| 0, 10)
									- parseInt(e.currentItem
													.css("paddingBottom")
													|| 0, 10));
							g.width(e.currentItem.innerWidth()
									- parseInt(e.currentItem.css("paddingLeft")
													|| 0, 10)
									- parseInt(e.currentItem
													.css("paddingRight")
													|| 0, 10))
				...
				if (c.p.sortable.options)
					a.extend(h, c.p.sortable.options);
				...
				if (c.p.sortable.exclude)
					h.items += ":not(" + c.p.sortable.exclude + ")";
		...
		columnChooser : function(b) {
			function j(f, k, p) {
				if (k >= 0) {
					var o = f.slice(), r = o.splice(k, Math
									.max(f.length - k, k));
					if (k > f.length)
						k = f.length;
					o[k] = p;
			...
			function c(f, k) {
				if (f)
					if (typeof f == "string")
						a.fn[f]
								&& a.fn[f].apply(k, a.makeArray(arguments)
												.slice(2));
					else
						a.isFunction(f)
								&& f.apply(k, a.makeArray(arguments).slice(2))
			}
			var h = this;
			if (!a("#colchooser_" + h[0].p.id).length) {
				...
				b = a.extend({
							...
							classname : null,
								...
								f && h.jqGrid("remapColumns", f, true)
							},
							msel : "multiselect",
							dlog : "dialog",
							...
							apply_perm : function() {
								a("option", e).each(function() {
									this.selected ? h.jqGrid("showCol",
											g[this.value].name) : h.jqGrid(
											"hideCol", g[this.value].name)
								});
								var f = [];
								...
								a.each(f, function() {
											delete l[g[parseInt(this, 10)].name]
										});
								a.each(l, function() {
											var k = parseInt(this, 10);
											f = j(f, k, k)
										});
								b.done && b.done.call(h, f)
							},
							cleanup : function(f) {
								c(b.dlog, d, "destroy");
								c(b.msel, e, "destroy");
								d.remove();
								f && b.done && b.done.call(h)
							},
							msel_opts : {}
						}, a.jgrid.col, b || {});
				if (a.ui)
					if (a.ui.multiselect)
						if (b.msel == "multiselect") {
							...
							b.msel_opts = a.extend(a.ui.multiselect.defaults,
									b.msel_opts)
						}
				...
				var g = h.jqGrid("getGridParam", "colModel"), i = h.jqGrid(
				...
				var n = a.isFunction(b.dlog_opts)
						? b.dlog_opts.call(h, b)
						: b.dlog_opts;
				c(b.dlog, d, n);
				n = a.isFunction(b.msel_opts)
						? b.msel_opts.call(h, b)
						: b.msel_opts;
				c(b.msel, e, n)
			}
		},
					...
					if (!j.p.treeGrid)
						if (a.fn.sortable) {
							...
							if (b.update && a.isFunction(b.update)) {
								...
								b._update_ = false;
							b.start = function(c, h) {
								a(h.item).css("border-width", "0px");
								a("td", h.item).each(function(g) {
									this.style.width = j.grid.cols[g].style.width
								});
								if (j.p.subGrid) {
									var d = a(h.item).attr("id");
									try {
										a(j).jqGrid("collapseSubGridRow", d)
									} catch (e) {
									}
								}
								b._start_ && b._start_.apply(this, [c, h])
							};
							b.update = function(c, h) {
								a(h.item).css("border-width", "");
								j.p.rownumbers === true
										&& a("td.jqgrid-rownum", j.rows).each(
												function(d) {
													a(this).html(d + 1)
												});
								b._update_ && b._update_.apply(this, [c, h])
							};
			...
			return this.each(function() {
				function j() {
					...
					a("tr.jqgrow:not(.ui-draggable)", c)
							.draggable(a.isFunction(d.drag) ? d.drag.call(a(c),
									d) : d.drag)
				}
				var c = this;
				if (c.grid)
					if (!c.p.treeGrid)
						if (a.fn.draggable && a.fn.droppable) {
							a("#jqgrid_dnd").html() === null
									&& a("body")
											.append("<table id='jqgrid_dnd' class='ui-jqgrid-dnd'></table>");
							if (typeof b == "string" && b == "updateDnD"
								...
								b = a.extend({
									...
									drop : function(d) {
										return a.extend({
											accept : function(e) {
												if (!a(e).hasClass("jqgrow"))
													return e;
												var g = a(e)
														.closest("table.ui-jqgrid-btable");
												if (g.length > 0
														&& a.data(g[0], "dnd") !== undefined) {
													e = a.data(g[0], "dnd").connectWith;
													return a.inArray("#"
																	+ this.id,
															e) != -1
															? true
								...
								if (b.connectWith) {
									...
									c.p.jqgdnd = true;
									for (var h = 0; h < b.connectWith.length; h++)
										a(b.connectWith[h]).droppable(a
						...
						if (j.grid && a.fn.resizable) {
							...
							b.stop = function(c, h) {
								a(j).jqGrid("setGridParam", {
									height : a("#gview_" + j.p.id
											+ " .ui-jqgrid-bdiv").height()
								});
								a(j).jqGrid("setGridWidth", h.size.width,
										b.shrinkToFit);
								b._stop_ && b._stop_.call(j, c, h)
							};
							b.alsoResize = b._alsoResize_
									? eval("("
											+ ("{'#gview_"
													...
													+ b._alsoResize_ + "':true}")
											+ ")")
									: a(".ui-jqgrid-bdiv", "#gview_" + j.p.id);
...
jQuery.extend($.fn.fmatter, {
	...
	translateCode : function(cellvalue, options, rowdata) {
		if (options.colModel.formatoptions
				&& options.colModel.formatoptions.codeClass) {
			var _codeClass = options.colModel.formatoptions.codeClass;
			$.codeCache = $.codeCache || {};
			if ($.codeCache[_codeClass] == null) {
				var _url = $.ctx + "/system1!listCode.action?codeClass="
						+ _codeClass;
				var _data = $.ajax({
							url : _url,
							async : false,
							...
							success : function(result) {
								...
								for (var i = 0; i < result.length; i++) {
									_codes[result[i].code] = result[i].codeDesc;
								}
								$.codeCache[_codeClass] = _codes;
							}
						});
			}
			return $.codeCache[_codeClass][cellvalue] || "";

		} else {
			return "没有设置codeClass";
		}
	},

	translateOrg : function(cellvalue, options, rowdata) {
		if ($.orgCache == undefined) {
			var _url = $.ctx + "/system3!organization.action";
			var _data = $.ajax({
						url : _url,
						async : false,
						dataType : "json",
						success : function(result) {
							$.orgCache = {};
							for (var i = 0; i < result.length; i++) {
								$.orgCache[result[i].code] = result[i].name;
							}
						}
					});
		}
		return $.orgCache[cellvalue] || "";
	}
		/*
		 * 修改、删除按钮添加 operations : function(cellvalue, options, rowdata) { if
		 * (options.colModel.formatoptions) { for (var i = 0; i <
		 * options.colModel.formatoptions.length; i++) { var _option =
		 * options.colModel.formatoptions[i]; return "<img src='"+ _option.icon + "'
		 * onclick='" + _option.action + "' title='" + _option.tip + "' />"; } } }
		 */
});
