 ...
 * Date: 2011-7-2
 * modifier: shenlj
 * reason: 修改方法:saveCell 中var cv = $.jgrid.checkValues(v,iCol,iRow,$t);
 *         增加参数iRow
 * 
 * ************************************************************************
 * Date: 2011-7-7
 * modifier: shenlj
 * reason: 修改方法:getChangedCells的返回值，为了不与业务数据冲突，把返回值中的属性名称"id"修改为"rowId"
 *         增加方法：saveEditingCell 用于保存正在被编辑的单元格
 *         
 * ************************************************************************
 * Date: 2011-8-3
 * modifier: 周营昭
 ...
 * Date: 2011-8-11
 * modifier: 周营昭
 * reason: saveCell时，如果校验有误，则return false。 
 * 
**/ 
/**
...
$.jgrid.extend({
	editCell : function (iRow,iCol, ed){
		return this.each(function (){
			...
			if ($t.p.colModel[iCol].editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
				...
				var opt = $.extend({"rowId": $t.rows[iRow].id}, $t.p.colModel[iCol].editoptions || {} ,{id:iRow+"_"+nm,name:nm});
		...
		this.each(function(){
			...
			if(fr !== null) {
				...
				if (v2 != $t.p.savedRow[fr].v){
					...
					var rowID = $t.rows[iRow].id == "" ? iRow: $t.rows[iRow].id;
					var cv = $.jgrid.checkValues(v,iCol,rowID,$t);
					...
					if(cv[0] === true) {
						...
						try {
							var positions = $.jgrid.findPos($($t.rows[iRow])[0]);
	...
	// 保存正在被编辑的单元格，用于填报 2011-07-07 shenlj
	saveEditingCell : function () {
		var $t= this;
		var $editinputID = $(".edit-cell",$t.grid).children().attr("id");
		if($editinputID != undefined && $editinputID != ""){
			var rowID = $editinputID.split("_")[0];
			var colID;
			var colName = $editinputID.split("_")[1];
      ...
      return this.saveCell(rowID,colID);
		}
	}
/// end  cell editing
});
