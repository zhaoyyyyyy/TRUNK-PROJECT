...
// 警告提示框
J.alertInfo = function(message) { // 弹出框函数
    var html = getHtml(message, 'ask.gif');
    var parentObj;
    if (frameElement.lhgDG != null && frameElement.lhgDG != undefined) {
        parentObj = frameElement.lhgDG;
        var dlg = parentdlg(parentObj, 'alertDialog', html);

    } else {
        var dlg = Firstdlg('alertDialog', html);
    }
...
// 询问提示框
J.confirmInfo = function(message, fn) {
    var html = getHtml(message, 'ask.gif');
    var parentObj;

    if (frameElement.lhgDG != null && frameElement.lhgDG != undefined) {
        parentObj = frameElement.lhgDG;
        var dlg = parentdlg(parentObj, 'confireDialog', html);

    } else {
        var dlg = Firstdlg('confireDialog', html);
    }
    dlg.ShowDialog();

    dlg.addBtn('submit', '确定', function() {
        fn.call(dlg,'点击了确定');
    });
    dlg.addBtn('close', '取消', dlg.cancel);
};
...
//显示的图片
//result:返回一个页面
getHtml = function(message, ImageName) {
    var html = '<table width="100%" style="vertical-align:middle" height="100%">'
			+ '<tr>'
			+ '<td  align="center">'
			+ '<img src="/mhjy/images/'
			+ ImageName
...
//ControlID:页面Id
//html:页面显示的内容
parentdlg = function(parentObj, ControlId, html) {
    var dlg = new parentObj.curWin.J.dialog({
        id: ControlId,
        cover: true, // 遮蔽层
        title: '提示', // 标题
        html: html, // 显示内容
        width: 250,
        height: 150,
        resize: false, // 是否允许拖动改变窗口大小，false：不可以
        rang: true
        // 是否在浏览器可视窗口内拖动，true：是
    });
    return dlg;
...
//ControlID:页面Id
//html:页面显示的内容
Firstdlg = function(ControlId, html) {
